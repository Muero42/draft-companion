...
window.PITTI_LIVE_DECISION_STATE=()=>{const c=lastDraftContext;if(!c?.scored)return null;const rows=visibleCoachCandidates(c.scored).filter(x=>!x.hardExcluded&&!x.recommendationBlocked).slice(0,10);return{version:APP_VERSION,current:c.current,next:c.next,profile:currentExpertProfile(),positionPanels:{...positionPanels},rows:rows.map(x=>({name:x.p.name,pos:x.p.pos,team:x.p.team,injury:x.p.injury||null,panel:x.r.rank,panelId:x.r.panelId||null,panelName:x.r.panel||null,individual:x.r.individual||[],adp:Number.isFinite(x.a)?x.a:null,ret:x.ret,returnConfidence:x.returnConfidence,confidence:x.confidence,score:x.score,action:x.action,loss:x.loss,outsideNormalCut:!!x.outsideNormalCut,role:pittiFantasyRole(x.p,c.players),arrows:pittiResearchArrows(x),reasons:Array.isArray(x.reasons)?x.reasons.slice():[],researchResidual:x.researchResidual||null}))};};
...
function decisionEvidenceExport(){
  const id=draftId(els.draftInput.value),all=loadDecisionFixtures(),rows=all.filter(f=>String(f.draftId||'')===String(id||''));
  ...
  return{format:'pitti-decision-evidence-v2',appVersion:APP_VERSION,createdAt:new Date().toISOString(),draftId:id||null,slot:Number(els.slot.value),mode:els.draftMode.value,fixtureCount:rows.length,resolvedCount:rows.filter(f=>f.chosenPlayer).length,overrideCount:summaries.filter(x=>x.override).length,wrSaturationRecommendationCount:summaries.filter(x=>x.wrSaturationFlag).length,qb2ViolationCount:summaries.filter(x=>x.qb2Violation).length,summaries,fixtures:rows};
}
