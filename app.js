...
const SEASON_SPECIAL_TEAMS_MODEL={
  ...
  k:{panelCandidates:['Jared Smola / Draft Sharks','Joe Bond — Fantasy Six Pack','Nathan Jahnke — PFF','Sean Koerner — Action Network'],qualityFloor:false,horizonWeeks:1,earlyAddWatch:{name:'Harrison Mevis',week1Projection:8.4,week1ProjectionRank:2,rotoballerWeek1Rank:10,policy:'WATCH_NOT_AUTO_ADD',rationale:'Strong Rams scoring environment and potential season-hold value; add only if live ownership/market-loss risk and weakest-roster-slot opportunity cost justify it.'}},
...
function renderSpecialTeamsBoard(){
  ...
  const kHtml=k.length?k.slice(0,7).map((x,i)=>'<div class="coach-row"><div><b>'+(i+1)+'. '+esc(x.p.name)+'</b><div class="tiny">K · FantasyPros W1 Consensus-Projektion · Sleeper frei</div></div><div><b>'+x.proj.toFixed(1)+' proj.</b></div></div>').join(''):'<div class="notice">Keiner der aktuell verifizierten Week-1-Kicker ist im LIVE Sleeper-FA-Pool; keine statische Liste darf Ownership überschreiben.</div>';
