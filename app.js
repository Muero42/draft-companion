...
function freezeDecisionFixture({draftId,current,returnPick,picks,mine,rankedAvailable,scored,rv2,mode,strategy,stress,teams,slot,fingerprint,map}){
  ...
  rows.push({
    ...
    modelVersion:'v11.8.0-rc4.79',researchResidualModel:RESEARCH_RESIDUAL_MODEL_VERSION,managerProfileHash:MANAGER_PROFILE_SOURCE_HASH,managerMapSnapshot:{...(map||{})},managerLiveStateSnapshot:JSON.parse(JSON.stringify(LIVE_MANAGER_ADAPTATION_STATE)),rng:{runs:rv2?.runs??900,seedBasis:`${current}|${returnPick??'end'}|${stress}`},
...
const RESEARCH_RESIDUAL_PRIORS={
  ...
  [norm('Malik Willis')]:{pos:'QB',expiresAt:Date.parse('2026-09-01T12:00:00Z'),components:[{kind:'expert_qualitative_upside',dir:1,strength:.78,confidence:.88,pricing:'POSSIBLY_UNPRICED',causal:'Fitzmaurice 27.08.: einer seiner drei Lieblings-QBs; außergewöhnlicher Rushing-Hebel plus verbesserter Passer. Aktuell als Miami QB1 geführt, während Markt/ECR deutlich später bleiben',invalidator:'Starterstatus kippt, designed/rushing volume bleibt niedrig oder Passing-Effizienz verhindert spielbaren Floor'}]},
