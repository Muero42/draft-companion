...
function renderAll(){if(!panels[activePanelId])activePanelId='standard';panelOptions(els.activePanel,activePanelId);for(const[pos,el]of [['QB',els.qbPanel],['RB',els.rbPanel],['WR',els.wrPanel],['TE',els.tePanel]])panelOptions(el,positionPanels[pos]||activePanelId);renderExperts();renderLog();renderPanelSummary();updateStatus()}
...
async function loadAllRanks(){saveCurrentPanel();const ids=[...new Set(Object.values(panels).flatMap(p=>Object.keys(p.members||{})))];if(!ids.length)throw new Error('Preset oder Expertenauswahl fehlt.');const missing=[];try{for(let i=0;i<ids.length;i++){const id=ids[i],name=experts.find(e=>e.id===id)?.name||id;els.dataStatus.className='notice';els.dataStatus.textContent=`Rankings werden geladen: ${i+1}/${ids.length} · ${name}`;const c=await loadExpertRanks(id);if(c.missing.length)missing.push(`${c.expertName}: ${c.missing.join('/')}`)}panelRanks={};for(const id of Object.keys(panels))panelRanks[id]=computePanel(id);store.set('v7_lastRankingUpdate',Date.now());persist();renderPanelSummary();updateStatus();if(missing.length){els.dataStatus.className='notice warn';els.dataStatus.textContent=`Rankings geladen. ${missing.length} Experte(n) haben für einzelne Positionen keine verwertbaren Daten; die verfügbaren Rankings bleiben aktiv.`}}finally{if(els.loadAllRanksBtn)els.loadAllRanksBtn.disabled=false}}
...
function panelReady(panelId){return Boolean(panelRanks[panelId]&&Object.keys(panelRanks[panelId]).length)}
function activePanelExperts(){const members=panels[activePanelId]?.members||{},total=Object.values(members).reduce((s,w)=>s+Number(w||0),0);return Object.entries(members).map(([id,w])=>({name:experts.find(e=>e.id===id)?.name||id,pct:total?Math.round(Number(w)/total*100):0})).sort((a,b)=>b.pct-a.pct)}
function renderPanelSummary(){if(!els.panelSummary)return;const active=panels[activePanelId]?.name||'Standard',selected=activePanelExperts(),overallReady=panelReady(activePanelId);const positions=['QB','RB','WR','TE'].map(pos=>{const ready=panelReady(positionPanels[pos]||activePanelId);return `<div class="panel-position"><b class="${ready?'ok':'warn'}">${ready?'✓':'–'}</b><span>${pos}</span></div>`}).join('');els.panelSummary.innerHTML=`<div class="panel-overview"><div class="panel-overview-head"><div><b>Aktives Panel: ${esc(active)}</b><small>${selected.length} ausgewählte Experten</small></div><strong class="${overallReady?'ok':'warn'}">${overallReady?'Bereit':'Fehlt'}</strong></div><div class="panel-experts">${selected.length?selected.map(x=>`<span class="panel-expert-chip">${esc(x.name)} ${x.pct}%</span>`).join(''):'<span class="tiny">Preset wird mit „Alles aktualisieren“ eingerichtet.</span>'}</div><div class="panel-position-grid"><div class="panel-position"><b class="${overallReady?'ok':'warn'}">${overallReady?'✓':'–'}</b><span>Overall</span></div>${positions}</div></div>`}
function updateStatus(){const rankTime=Number(store.get('v7_lastRankingUpdate',0)),hours=rankTime?(Date.now()-rankTime)/3600000:null,hasKey=Boolean(els.apiKey.value.trim()),panelsReady=panelReady(activePanelId)&&['QB','RB','WR','TE'].every(pos=>panelReady(positionPanels[pos]||activePanelId)),dataReady=hasKey&&experts.length>0&&panelsReady;els.onlineState.textContent=navigator.onLine?'Online':'Offline';els.onlineState.className=navigator.onLine?'ok':'bad';els.rankingAge.textContent=hours==null?'Fehlt':hours<1?`${Math.max(1,Math.round(hours*60))} Min.`:hours<24?`${Math.round(hours)} Std.`:`${Math.floor(hours/24)} Tag(e)`;els.rankingAge.className=hours==null?'bad':hours>24?'warn':'ok';els.adpCount.textContent=Object.keys(adp).length?String(Object.keys(adp).length):'Optional';els.adpCount.className=Object.keys(adp).length?'ok':'warn';els.qualityMini.textContent=lastDraftContext?'Bereit':dataReady?'Daten bereit':'Fehlt';els.qualityMini.className=lastDraftContext||dataReady?'ok':'warn';const missing=[];if(!hasKey)missing.push('API-Key');if(!experts.length)missing.push('Experten');if(!panelsReady)missing.push('Panel-Rankings');els.dataStatus.className=`notice ${missing.length?'warn':'ok'}`;els.dataStatus.textContent=missing.length?`Noch erforderlich: ${missing.join(' · ')}.`:`Daten bereit: ${experts.length} Experten und alle Positionspanels sind aktuell.`;if(Object.keys(adp).length){els.adpStatus.className='notice ok';els.adpStatus.textContent=`${Object.keys(adp).length} Sleeper-ADPs aktiv · Quelle: ${adpMeta.source||'verifizierter Import'}.`}else{els.adpStatus.className='notice warn';els.adpStatus.textContent='Sleeper-ADP optional: Expertenrankings funktionieren vollständig; Reach und Return werden ohne ADP nicht scheinpräzise berechnet.'}renderPanelSummary()}
...
function backup(){return{format:'draft-companion-v7',version:'9.0.0',createdAt:new Date().toISOString(),season:els.season.value,scoring:els.scoring.value,experts,panels,activePanelId,positionPanels,rankCache,panelRanks,adp,adpMeta,decisionLog,draft:els.draftInput.value,slot:els.slot.value}}
...
els.refreshAllBtn.onclick=async()=>{els.refreshAllBtn.disabled=true;els.refreshAllBtn.textContent='Aktualisiere …';try{if(!els.apiKey.value.trim())throw new Error('API-Key unter „Erweitert“ eintragen.');await loadExperts();if(!Object.values(panels).some(p=>Object.keys(p.members||{}).length))applyPreset();await loadAllRanks();await loadSleeperAdpFromFantasyPros();const sleeperPlayers=await fetch('https://api.sleeper.app/v1/players/nfl',{cache:'no-store'});if(!sleeperPlayers.ok)throw new Error('Sleeper-Spielerdaten nicht erreichbar.');store.set('v7_lastFullUpdate',Date.now());persist();updateStatus()}catch(e){els.dataStatus.className='notice bad';els.dataStatus.textContent=e.message}finally{els.refreshAllBtn.disabled=false;els.refreshAllBtn.textContent='Alles aktualisieren'}};
...
for(const section of [els.dataSection,els.draftSection]){const heading=section?.querySelector('.section-toggle');const toggle=()=>{const collapsed=section.classList.toggle('is-collapsed');heading?.setAttribute('aria-expanded',String(!collapsed))};if(heading){heading.addEventListener('click',e=>{if(e.target.closest('button'))return;toggle()});heading.addEventListener('keydown',e=>{if(e.key==='Enter'||e.key===' '){e.preventDefault();toggle()}})}}
...
renderAll();renderPanelSummary();setAuto();updateStatus();
