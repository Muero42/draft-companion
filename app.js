...
async function fetchSeasonLeagueState(draft){
  ...
  const [league,rosters,users]=await Promise.all([
    jf(S+'/league/'+encodeURIComponent(leagueId)+'?_='+bust,'Sleeper Liga',7000),
    jf(S+'/league/'+encodeURIComponent(leagueId)+'/rosters?_='+bust,'Sleeper Kader',7000),
    jf(S+'/league/'+encodeURIComponent(leagueId)+'/users?_='+bust,'Sleeper Manager',7000).catch(()=>[])
  ]);
  ...
  try{store.setText('v118_seasonLeagueState',JSON.stringify({generated_at:state.generated_at,league_id:leagueId,faab_budget:faabBudget,transaction_round:transactionRound,rosters:league_rosters,transactions:transactions.map(t=>({transaction_id:t.transaction_id,type:t.type,status:t.status,roster_ids:t.roster_ids,settings:t.settings,adds:t.adds,drops:t.drops,created:t.created}))}));}catch{}
  return state;
}
function seasonRosterRows(season,players,draftMine){if(!season?.ok||!season.my_roster)return null;const draftById=new Map((draftMine||[]).map(pk=>[String(pk.player_id),pk])),reserve=new Set((season.my_roster.reserve||[]).map(String)),taxi=new Set((season.my_roster.taxi||[]).map(String));return[...new Set((season.my_roster.players||[]).map(String))].map(pid=>{const p=sleeperPlayerRow(pid,players),base=draftById.get(pid);return{pk:base||{player_id:pid,pick_no:999,metadata:{}},p:{...p,injury:reserve.has(pid)?(p.injury||'IR/RESERVE'):p.injury},r:rankFor(p.name,p.pos),a:adpFor(p.name),seasonStatus:reserve.has(pid)?'RESERVE':taxi.has(pid)?'TAXI':'ACTIVE'};});}
...
function pruneNonCriticalStorageForRankWrite(){
  // Rank caches are rebuildable. Research, return validation, decision fixtures and
  // current Season/Weekly evidence are not, so there is nothing else safe to evict.
...
function persistExpertRankCache(expertId,result){
  ...
  try{localStorage.setItem(key,payload);return{ok:true,recovered:false}}
    ...
    catch(second){pruneNonCriticalStorageForRankWrite();return{ok:false,error:second}}
  }
}
...
function summarizeWeeklyProjectionPayload(payload,{position,season,week,status,retryAfterMs=null}){
  ...
  return{position,status,retryAfterMs,season:Number(season),week:Number(week),queryShape:'EXPLICIT_WEEK_POSITION_ROS_OMITTED',count,overTen:count>10,identityCount,pointsHalfCount,coveragePct:Math.round(coverage*1000)/10,metadata:weeklyProjectionMetadata(payload),sourceChronology,result,reason:reason||(!sufficient?'INSUFFICIENT_COVERAGE':'')};
}
function weeklyProjectionFailure(position,season,week,error){
  ...
  return{position,status:Number.isFinite(status)?status:null,retryAfterMs:Number.isFinite(Number(error?.retryAfterMs))?Number(error.retryAfterMs):null,season:Number(season),week:week!=null&&Number.isFinite(Number(week))?Number(week):null,queryShape:'EXPLICIT_WEEK_POSITION_ROS_OMITTED',count:0,overTen:false,identityCount:0,pointsHalfCount:0,coveragePct:0,metadata:[],result:'FAILED',reason:code};
}
function weeklyProjectionConsumerDiagnostic(payloads,{season,week,sleeperPlayers,seasonState,verifiedAt=Date.now()}={}){
  ...
  const snapshot=api.buildSnapshot({season,week,scoring:'HALF_PPR',projectionPayloads:payloads,sleeperPlayers,verifiedAt}),positions=WEEKLY_PROJECTION_POSITIONS.map(position=>({position,...snapshot.lanes.projections.coverage.positions[position]}));
  const usableIds=new Set(snapshot.records.filter(record=>record.metric==='projected_points'&&api.weeklyRecordChronology(record,{season,week,scoring:'HALF_PPR'},verifiedAt)).map(record=>String(record.playerId)));
  const reserve=new Set([...(seasonState?.my_roster?.reserve||[]),...(seasonState?.my_roster?.taxi||[])].map(String)),activeIds=[...new Set((seasonState?.my_roster?.players||[]).map(String))].filter(id=>!reserve.has(id)&&WEEKLY_PROJECTION_POSITIONS.includes(String(sleeperPlayers[id]?.position||'').toUpperCase()));
  const unusable=activeIds.filter(id=>!usableIds.has(id)).map(id=>({id,name:sleeperPlayers[id]?.full_name||[sleeperPlayers[id]?.first_name,sleeperPlayers[id]?.last_name].filter(Boolean).join(' ')||'UNKNOWN',position:String(sleeperPlayers[id]?.position||'').toUpperCase()}));
  return{available:true,snapshotStatus:snapshot.status,laneStatus:snapshot.lanes.projections.status,validation:api.validateSnapshot(snapshot,{season,week,scoring:'HALF_PPR'},verifiedAt),positions,roster:{activeSkillCount:activeIds.length,usableCount:activeIds.length-unusable.length,unusable}};
}
async function runAuthenticatedWeeklyProjectionDiagnostic(){
  ...
  for(const position of WEEKLY_PROJECTION_POSITIONS){
    ...
    try{
      ...
      if(!response.ok)rows.push(weeklyProjectionFailure(position,season,week,{status:response.status,retryAfterMs:response.retryAfterMs}));
      else{rows.push(summarizeWeeklyProjectionPayload(response.data,{position,season,week,status:response.status,retryAfterMs:response.retryAfterMs}));payloads[position]={providerPayload:response.data,requestProvenance:{season,week,position,scope:'WEEKLY',queryShape:'EXPLICIT_WEEK_POSITION_ROS_OMITTED'}};}
  ...
  const diagnosticContext=typeof lastDraftContext==='undefined'?null:lastDraftContext,consumer=weeklyProjectionConsumerDiagnostic(payloads,{season,week,sleeperPlayers:diagnosticContext?.players,seasonState:diagnosticContext?.season});
  return{rows,classification:failed.length?'INSUFFICIENT':'SUFFICIENT',reason:failed.map(row=>`${row.position}:${row.reason}`).join(','),consumer};
}
...
function diagnosticRetryAfter(ms){return ms!=null&&Number.isFinite(Number(ms))&&Number(ms)>=0?Math.round(Number(ms)/1000):null}
function sanitizedProjectionDiagnostic(report){
  const byPosition=Object.fromEntries((report?.rows||[]).map(row=>[row.position,{httpStatus:Number.isInteger(row.status)?row.status:null,retryAfterSeconds:diagnosticRetryAfter(row.retryAfterMs),week:row.week,position:row.position,rosPresent:false,scoringParameterPresent:false,queryShape:row.queryShape,sourceRows:row.count,numericStatsPointsHalf:row.pointsHalfCount,chronologyPrecision:row.sourceChronology?.sourceTimePrecision||null,sourcePublishedAt:row.sourceChronology?.sourcePublishedAt||null,sourcePublishedDate:row.sourceChronology?.sourcePublishedDate||null,result:row.result,acquisitionReason:row.reason||null}]));
...
async function runCurrentWeekRankDiagnostic({season,week,sleeperPlayers,directoryPayloadSink}={}){
  ...
  settled.forEach((result,index)=>{if(result.status==='rejected'){const position=result.reason?.position||WEEKLY_PROJECTION_POSITIONS[index];requests[position]??={httpStatus:Number.isInteger(result.reason?.status)?result.reason.status:null,retryAfterSeconds:diagnosticRetryAfter(result.reason?.retryAfterMs),week,position,scoring:'HALF',acquisitionReason:result.reason?.code||'NETWORK'}}});
...
async function runCanonicalGameContextDiagnostic({season,week}={}){
  ...
  try{const response=await fetch(path,{cache:'no-store'});httpStatus=response.status;let data=null;try{data=await response.json()}catch{}upstreamStatus=Number.isInteger(data?.upstreamStatus)?data.upstreamStatus:null;failureType=typeof data?.failureType==='string'?data.failureType:null;snapshot=api&&(response.ok&&data)?api.buildSnapshot({...data,verifiedAt:Date.now()}):api?.failureSnapshot({season,week,httpStatus:response.status,reason:response.ok?'MALFORMED_JSON':failureType||'HTTP_ERROR',verifiedAt:Date.now()});}
  catch{snapshot=api?.failureSnapshot({season,week,reason:'FETCH_ERROR',verifiedAt:Date.now()});}
  if(!api||!snapshot)return{request:path,httpStatus,status:'UNAVAILABLE',validation:'GAME_CONTEXT_MODULE_MISSING',coverage:{sourceEvents:0,acceptedGames:0,acceptedTeams:0,storedGames:0},games:[],rejections:[]};
  const validation=api.validateSnapshot(snapshot,{season,week},Date.now()),games=(snapshot.games||[]).slice(0,32).map(game=>({eventId:game.eventId,homeTeam:game.homeTeam,awayTeam:game.awayTeam,opponents:{[game.homeTeam]:game.awayTeam,[game.awayTeam]:game.homeTeam},roof:game.roof,weatherStatus:game.weather?.status||'UNAVAILABLE',weatherReason:game.weather?.reason||null}));
  return{request:path,httpStatus,upstreamStatus,failureType,status:snapshot.status,validation:validation.ok?'PASS':validation.reason,coverage:{sourceEvents:snapshot.coverage?.sourceEvents||0,acceptedGames:snapshot.coverage?.acceptedGames||0,acceptedTeams:snapshot.coverage?.acceptedTeams||0,storedGames:snapshot.games?.length||0},games,rejections:(snapshot.rejections||[]).slice(0,32).map(row=>({eventId:row.eventId??null,reason:row.reason||'UNKNOWN',httpStatus:Number.isInteger(row.httpStatus)?row.httpStatus:null}))};
}
async function runPhysicalEvidenceLaneDiagnostic(){
  ...
  const rankDirectory={},context={season,week,sleeperPlayers:lastDraftContext?.players},projectionPromise=runAuthenticatedWeeklyProjectionDiagnostic(),rankPromise=week?runCurrentWeekRankDiagnostic({...context,directoryPayloadSink:rankDirectory}):Promise.resolve({status:'UNAVAILABLE',reason:weekReason,positions:{}}),selectedPromise=week?rankPromise.then(()=>runSelectedPittiPanelDiagnostic({...context,directoryPayloads:rankDirectory.payloads||{}})):Promise.resolve({status:'UNAVAILABLE',reason:weekReason,positions:{}}),gamePromise=week?runCanonicalGameContextDiagnostic({season,week}):Promise.resolve({request:null,httpStatus:null,status:'UNAVAILABLE',validation:weekReason,coverage:{sourceEvents:0,acceptedGames:0,acceptedTeams:0,storedGames:0},games:[],rejections:[]});
  const [projectionResult,rankResult,selectedResult,gameResult]=await Promise.allSettled([projectionPromise,rankPromise,selectedPromise,gamePromise]),value=(result,fallback)=>result.status==='fulfilled'?result.value:fallback,weeklyProjections=sanitizedProjectionDiagnostic(value(projectionResult,{classification:'UNAVAILABLE',reason:'COLLECTION_FAILED',rows:[]})),expertRanks=value(rankResult,{status:'UNAVAILABLE',reason:'COLLECTION_FAILED',positions:{}}),selectedPittiPanel=value(selectedResult,{status:'UNAVAILABLE',reason:'COLLECTION_FAILED',positions:{}});
...
function renderAll(){
  if(!panels[activePanelId])activePanelId='standard';
  if(els.activePanel)panelOptions(els.activePanel,activePanelId);
  for(const[pos,el]of [['QB',els.qbPanel],['RB',els.rbPanel],['WR',els.wrPanel],['TE',els.tePanel]])if(el)panelOptions(el,positionPanels[pos]||activePanelId);
  if(els.expertSearch&&els.expertsList)renderExperts();
  if(els.decisionLog)renderLog();
  updateStatus();
}
...
function seasonBootstrapWatchdogArm(){
  const token=String(Date.now())+'-'+Math.random().toString(36).slice(2);store.setText('v118_seasonBootstrapToken',token);
  setTimeout(()=>{
    if(store.text('v118_seasonBootstrapToken','')!==token)return;
    if(els.seasonLiveStateStatus&&els.seasonLiveStateStatus.textContent.includes('geladen')){els.seasonLiveStateStatus.className='notice bad';els.seasonLiveStateStatus.textContent='Kader-Aktualisierung fehlgeschlagen · Startup-Watchdog · keine Aktion freigegeben.';}
    if(els.rosterStatus&&els.rosterStatus.textContent==='Live-Kader wird geladen…'){els.rosterStatus.className='notice warn';els.rosterStatus.textContent='Season Auto-Sync FAIL-CLOSED · Startup-Watchdog · keine FA-Aktion freigegeben.';}
  },SEASON_BOOTSTRAP_WATCHDOG_MS);
  return token;
}
...
/* rc4.176 — no premature Season execution: initialize the full module first. */
...
function seasonHorizonSplit(drop,fa,structural=null){
  const d=postDraftOpportunityProxy(drop),f=postDraftOpportunityProxy(fa);
  const upside=postDraftUpsideProxy(fa)-postDraftUpsideProxy(drop);
  const weeklyEvidence=freshAcquisitionEvidence(fa),dropWeeklyEvidence=freshAcquisitionEvidence(drop),roleEvents=weeklyEvidence.events+dropWeeklyEvidence.events;
  // THIS WEEK is the verified legal-lineup projection delta already computed by the
...
function postDraftSwapScore(drop,fa,ctx){
  ...
  const structural=seasonAcquisitionDecision(drop,fa,ctx.rows||[],ctx.season);
  const rosterUtility=Number.isFinite(structural.utility)?structural.utility:0;
...
function renderRosterFaAudit(rows,rankedAvailable,draftComplete,opts={render:true}){
  ...
  els.rosterFaList.innerHTML=`<div class="coach-section-title">Konkrete Add/Drop-Paare</div>`+surfaced.map((x,i)=>{
    const why=[`THIS WEEK ${Number.isFinite(x.horizons.weekly)?`${x.horizons.weekly>=0?'+':''}${x.horizons.weekly.toFixed(1)} Pkt`:'– (verifizierte Weekly-Projektionen fehlen)'}`,`ROS ${Number.isFinite(x.horizons.ros)?`${x.horizons.ros>=0?'+':''}${x.horizons.ros.toFixed(1)} Value`:'– (aktueller vergleichbarer Trade-Value fehlt)'}`,`Kontingenz ${Number.isFinite(x.horizons.contingent)?`${x.horizons.contingent>=0?'+':''}${x.horizons.contingent.toFixed(1)}`:'– (frische Rollen-Evidence fehlt)'}`,`Panel Δ ${x.panelDelta>=0?'+':''}${x.panelDelta.toFixed(1)}`,`Roster ${x.rosterUtility>=0?'+':''}${x.rosterUtility.toFixed(1)}`];
    ...
    return `<article class="coach"><div class="coach-head"><div><h3>${i+1}. ${x.action}: ${esc(x.fa.p.name)} → für ${esc(x.drop.p.name)}</h3><div class="tiny">ADD ${x.fa.p.pos} ${x.fa.p.team} · DROP ${x.drop.p.pos} ${x.drop.p.team} · Confidence ${x.confidence}% · ${x.action==='CLEAR ADD'?'sofort prüfen':'monitor / kein Rush'}</div></div><div class="score">${x.score.toFixed(1)}</div></div><div class="tiny">Player Quality: FA Panel ${(Number.isFinite(x.fa.r?.rank)?x.fa.r.rank.toFixed(1):'–')} vs Roster ${Number.isFinite(x.drop.r?.rank)?x.drop.r.rank.toFixed(1):'–'} · ${why.join(' · ')}</div><div class="tiny">Freshness: ${esc(fresh)} · Provenance: ${esc(provenance)}</div><div class="tiny">Invalidator/Recheck: ${esc(invalidator)}</div></article>`;
  }).join('');
}
...
function waiverMarketSummary(pair,players){
  const season=lastDraftContext?.season,opponents=waiverOpponentMarket(pair.fa,players),budget=Number(season?.faab_budget),mine=(season?.league_rosters||[]).find(r=>Number(r.roster_id)===Number(season?.roster_id));
  const allNine=opponents.length===9,evaluated=opponents.filter(x=>x.evaluation_status==='VERIFIED'),budgetsVerified=Number.isFinite(budget)&&budget>0&&opponents.every(x=>Number.isFinite(x.faab_remaining));
  const sufficient=allNine&&evaluated.length===9&&budgetsVerified;
  const marketHigh=sufficient?Math.max(0,...opponents.map(x=>x.bid_high_pct)):null;
...
function renderWaiverWorkspace(draftComplete){
  ...
  const cards=q.map((x,i)=>{
    ...
    const provenance=[x.structural?.reason,h.rosSource?`ROS ${h.rosSource.sourceId} · ${new Date(h.rosSource.publishedAt).toLocaleString('de-DE')}`:'ROS nicht verfügbar',`Live-State ${new Date(lastDraftContext.season.generated_at).toLocaleTimeString('de-DE')}`].filter(Boolean).join(' · ');
    return `<article class="coach waiver-decision-card"><div class="coach-head"><div><h3>${i+1}. ${action}: ADD ${esc(x.fa.p.name)} / DROP ${esc(x.drop.p.name)}</h3><div class="tiny">${x.fa.p.pos} ${x.fa.p.team} für ${x.drop.p.pos} ${x.drop.p.team} · ${urgency} · Confidence ${x.confidence}%</div></div><div class="score">${x.score.toFixed(1)}</div></div><div class="tiny"><b>Horizont:</b> ${horizon}</div><div class="tiny"><b>FAAB:</b> ${faab}</div><div class="tiny"><b>Stärkste Konkurrenz:</b> ${rivals||'nicht belastbar verfügbar'}</div><div class="tiny"><b>Warum:</b> ${esc(provenance)}</div><div class="tiny"><b>Invalidator:</b> ${esc(invalidator)}</div><div class="tiny">Keine automatische Transaktion.</div></article>`;
  }).join('');
  const skill=cards||'<div class="notice ok"><b>SKILL-POSITION HOLD</b> · Kein evidenzgestützter ADD/DROP-Swap. Fehlende Daten werden nicht durch Draft-Ranks oder Scheingenauigkeit ersetzt.</div>';
  els.waiverList.innerHTML='<div class="coach-section-title">JETZT ENTSCHEIDEN · SKILL-POSITIONEN</div>'+skill+renderQbOpportunityBoard()+renderSpecialTeamsBoard();
}
...
function renderSeasonActionBoard(draftComplete){
  ...
  els.seasonActionStatus.className='notice';els.seasonActionStatus.textContent='Live / News · verifizierte Ereignisse und vorbereitete Rollenfolgen · keine automatische Transaktion';
...
function seasonRosterPlayerRows(rr,players){
  const reserve=new Set((rr?.reserve||[]).map(String)),taxi=new Set((rr?.taxi||[]).map(String));
  return [...new Set([...(rr?.players||[]),...reserve,...taxi].map(String))].map(pid=>{const p=sleeperPlayerRow(pid,players),r=rankFor(p.name,p.pos),weekly=weeklyLineupEvidence(p),currentRank=weekly?.freshEnough&&Number.isFinite(weekly?.consensus)?Number(weekly.consensus):null;return{p,r,currentRank,seasonStatus:reserve.has(pid)?'RESERVE':taxi.has(pid)?'TAXI':'ACTIVE'};});
}
function waiverOpponentMarket(target,players){
  ...
  return (season.league_rosters||season.rosters||[]).filter(rr=>Number(rr.roster_id)!==mine).map(rr=>{
    const rows=seasonRosterPlayerRows(rr,players),active=rows.filter(x=>x.seasonStatus==='ACTIVE'),targetPlayer=target.p||target,targetPos=String(targetPlayer?.pos||'').toUpperCase();
    const positional=active.filter(x=>String(x.p.pos).toUpperCase()===targetPos).sort((a,b)=>(a.currentRank??999)-(b.currentRank??999));
    const targetWeekly=weeklyLineupEvidence(targetPlayer);
    const targetRank=targetWeekly?.freshEnough&&Number.isFinite(targetWeekly?.consensus)?Number(targetWeekly.consensus):null;
    const best=positional[0]?.currentRank,rankUpgrade=Number.isFinite(best)&&Number.isFinite(targetRank)?Math.max(0,best-targetRank):0,before=seasonProjectionLineup(active,season),after=seasonProjectionLineup(active.concat({...target,p:targetPlayer,seasonStatus:'ACTIVE'}),season);
    ...
    const compatible=tradeStarterSlots(season).filter(slot=>seasonSlotEligible(slot,targetPos)).length;
    const scarcity=Math.max(0,compatible-positional.length);
    const liveNeed=Number.isFinite(lineupGain)?lineupGain*.65+rankUpgrade/14+(positional.length===0?2.5:0)+scarcity*.45-benchCost*.35:null;
    const evaluationStatus=Number.isFinite(lineupGain)&&Number.isFinite(remaining)?'VERIFIED':'UNAVAILABLE';
    const need=evaluationStatus==='VERIFIED'?clamp(liveNeed+historyPrior+txPrior,0,10):0;
    const claimProbability=evaluationStatus==='VERIFIED'?clamp(Math.round(5+need*5),5,55):null;
    const bidMid=evaluationStatus==='VERIFIED'?clamp(need*.9,0,12):null,bidLow=Number.isFinite(bidMid)?Math.max(0,Math.floor(bidMid-1.5)):null,bidHigh=Number.isFinite(bidMid)?Math.min(15,Math.ceil(bidMid+2)):null;
    const needLabel=evaluationStatus!=='VERIFIED'?'Bedarf unbekannt':need>=6?'hoher Bedarf':need>=3?'mittlerer Bedarf':'geringer Bedarf';
    return{roster_id:Number(rr.roster_id),manager_name:managerName,position:targetPos,position_players:positional.map(x=>x.p.name),need,need_label:needLabel,live_need:liveNeed,lineup_gain:lineupGain,manager_history_prior:historyPrior,current_season_bid_prior:txPrior,faab_remaining:remaining,claim_probability:claimProbability,bid_low_pct:bidLow,bid_high_pct:bidHigh,evaluation_status:evaluationStatus};
...
function renderQbOpportunityBoard(){
  ...
  const rows=qbs.map(x=>{
    const alternatives=(ctx.seasonRows||[]).map(drop=>({drop,...seasonAcquisitionDecision(drop,x,ctx.seasonRows,season)})).filter(d=>d.action!=='HOLD').sort((a,b)=>b.utility-a.utility);
    const structural=alternatives[0],drop1=structural?.drop,drop2=structural?.secondDrop;
...
function tradeStarterSlots(season=lastDraftContext?.season){
...
function renderTradeWorkspace(picks,players,userSlot,teams,draftComplete){
  ...
  els.tradeList.innerHTML=actionableTargets.length?`<div class="coach-section-title">KONKRETE ANGEBOTE · REVIEW VOR SENDEN</div>`+actionableTargets.slice(0,8).map((t,i)=>{
    ...
    const opponentReason=`${esc(managerLabel)} gewinnt ${d.opponentGain.toFixed(1)} verifizierte Startaufstellungs-Punkte; Paketwert ${d.giveValue.toFixed(1)} für ${d.getValue.toFixed(1)} und Bedarf ${esc(t.oppNeeds[0]?.pos||'Roster-Fit')} wird adressiert.`;
...
function seasonEvidenceContext(season=lastDraftContext?.season){return{season:Number(season?.league?.season),week:Number(season?.current_nfl_week??season?.transaction_round??season?.league?.settings?.leg),scoring:'HALF_PPR'};}
...
function seasonEvidenceValue(records,playerId,metric,context,now=Date.now()){
  ...
  if(new Set(valid.map(x=>x.value)).size>1)return{status:'CONFLICT',value:null};
...
function seasonGameContext(p,season=lastDraftContext?.season){
  ...
  const valid=current.filter(x=>x.status==='VERIFIED'&&Number.isFinite(x.verifiedAt)&&x.publishedAt<=x.verifiedAt&&x.verifiedAt<=now&&/^https:\/\//.test(x.sourceUrl||''));
  const conflict=current.some(x=>x.conflict)||new Set(valid.map(x=>JSON.stringify([x.opponent,x.dome,x.weatherSummary]))).size>1;
  const e=conflict?null:valid.sort((a,b)=>b.verifiedAt-a.verifiedAt)[0];
...
function seasonRosterAuthority(rows,season,rosterId){
  ...
  if(!roster||!Array.isArray(roster.players)||!Array.isArray(rows))return false;
  const inactive=new Set([...(roster.reserve||[]),...(roster.taxi||[])].map(String));
  const expected=roster.players.map(String).filter(id=>!inactive.has(id)),active=rows.filter(x=>x?.seasonStatus==='ACTIVE').map(x=>String(x.p?.id));
  return new Set(active).size===active.length&&new Set(expected).size===expected.length&&expected.length===active.length&&expected.every(id=>active.includes(id)&&Number(season.ownership?.[id]?.roster_id)===Number(rosterId)&&!season.ownership[id].reserve&&!season.ownership[id].taxi);
}
function seasonLegalDrop(drop,fa,rows){
  if(drop?.seasonStatus!=='ACTIVE'||!drop?.p||!fa?.p||String(drop.p.id)===String(fa.p.id))return false;
  const d=drop.p.pos,f=fa.p.pos;
  if(d==='K'||f==='K')return d==='K'&&f==='K';
  if(['DEF','DST'].includes(d)||['DEF','DST'].includes(f))return ['DEF','DST'].includes(d)&&['DEF','DST'].includes(f);
...
function seasonAcquisitionDecision(drop,fa,rows,season){
  ...
  if(before.status!=='VERIFIED'||after.status!=='VERIFIED')return result;
  if(after.assignments.filter(x=>x.player).length<before.assignments.filter(x=>x.player).length){result.reason='Tausch würde einen belegten Starter-Slot verlieren';return result;}
  const capacity=season.league.roster_positions.filter(x=>!SLEEPER_NON_STARTER_SLOTS.has(String(x).toUpperCase())||String(x).toUpperCase()==='BN').length;
  if(next.length>capacity){result.reason='Aktive Kaderkapazität überschritten';return result;}
  const missingDst=season.league.roster_positions.some(x=>['DST','DEF'].includes(String(x).toUpperCase()))&&!next.some(x=>['DST','DEF'].includes(x.p.pos));
  let futureCost=0;
  ...
  result.lineupGain=after.score-before.score;
  result.utility=result.lineupGain-futureCost-(result.sameBye?2:0);
  result.action=result.utility>=2?'CLEAR ADD':result.utility>0?'WATCH':'HOLD';
  result.reason='Legaler Lineup-Gewinn '+result.lineupGain.toFixed(1)+' · künftige Kapazitätskosten '+futureCost.toFixed(1)+(result.sameBye?' · gleiche Bye':'');
  return result;
}
function seasonTradeDecision(mine,opponent,gives,gets,season){
  const result={actionable:false,status:'UNAVAILABLE',ourGain:null,opponentGain:null,acceptance:null,giveValue:null,getValue:null,fairnessPct:null,valueSource:null,revealedPreferencePenalty:0};
  if(!seasonLiveAuthority(season)||!gives.length||!gets.length||gives.length>2||gets.length>2)return result;
  if([...gives,...gets].some(x=>x.seasonStatus!=='ACTIVE'))return result;
  if(new Set([...gives,...gets].map(x=>String(x.p?.id))).size!==gives.length+gets.length)return result;
  if(gives.some(x=>!mine.includes(x))||gets.some(x=>!opponent.includes(x)))return result;
  const ourId=season.my_roster?.roster_id,oppId=season.ownership?.[String(gets[0].p.id)]?.roster_id;
  if(Number(ourId)===Number(oppId)||!seasonRosterAuthority(mine,season,ourId)||!seasonRosterAuthority(opponent,season,oppId))return result;
  const nextMine=mine.filter(x=>!gives.includes(x)).concat(gets),nextOpp=opponent.filter(x=>!gets.includes(x)).concat(gives);
...
function seasonNewsReactions(events,graphs,season,now=Date.now()){
  ...
  for(const e of Array.isArray(events)?events:[]){
    if(!e?.playerId||!e.eventKey||!Number.isFinite(e.eventOccurredAt))continue;
    const key=[e.playerId,e.eventKey].join('|'),group=groups.get(key)||[];group.push(e);groups.set(key,group);
  }
  return [...groups].map(([key,group])=>{
    const current=group.filter(e=>e.sourcePublishedAt>0&&e.sourcePublishedAt<=now&&now-e.sourcePublishedAt<=86400000&&e.eventOccurredAt<=e.sourcePublishedAt&&now-e.eventOccurredAt<=86400000&&/^https:\/\//.test(e.sourceUrl||''));
    const confirmed=current.filter(e=>e.verified===true&&e.sourceTier===0&&e.sourceOriginality==='primary'&&e.verifiedAt>=e.sourcePublishedAt&&e.verifiedAt<=now);
    const e=confirmed[0]||current[0]||group[0],roles=(Array.isArray(graphs)?graphs:[]).filter(g=>g&&String(g.playerId)===String(e.playerId)&&Number.isFinite(g.verifiedAt)&&g.verifiedAt<=now&&now-g.verifiedAt<=86400000),validRoles=roles.filter(g=>g.status==='VERIFIED'&&/^https:\/\//.test(g.sourceUrl||''));
    const conflict=current.some(e=>e.conflict)||new Set(confirmed.map(e=>e.status)).size>1||roles.some(g=>g.conflict)||new Set(validRoles.map(g=>JSON.stringify(g.successors))).size>1;
    const graph=validRoles.sort((a,b)=>b.verifiedAt-a.verifiedAt)[0];
    const verified=!!confirmed.length&&['OUT','INACTIVE','IR','PUP','SUSPENDED','RELEASED'].includes(e.status)&&!conflict&&!!graph&&seasonLiveAuthority(season,now);
    const successors=verified?(Array.isArray(graph.successors)?graph.successors:[]).filter(s=>s?.playerId&&s.functionalRole&&/^https:\/\//.test(s.roleEvidenceUrl||'')&&s.confidence>=.7&&s.confidence<=1).map(s=>({...s,availability:Object.hasOwn(season.ownership,String(s.playerId))?'OWNED':'FREE_AGENT'})):[];
    return{key,playerId:e.playerId,status:conflict?'CONFLICT':verified?'VERIFIED_IMPACT':'PROVISIONAL_WATCH',successors,actionable:false,firstSeen:Math.min(...group.map(x=>x.observedAt||now)),publishedAt:e.sourcePublishedAt||null,ingestedAt:e.ingestedAt||now,verifiedAt:verified?e.verifiedAt:null,decisionAt:now,invalidator:graph?.invalidator||'Primärquelle, Rollenwechsel oder Ownership neu prüfen'};
  });
}
function seasonNewsHtml(){
  const reactions=seasonNewsReactions(loadResearchEvents(),store.get('v190_roleGraphs',[]),lastDraftContext?.season);
  return reactions.length?reactions.slice(-12).map(e=>{
    const c=lastDraftContext,actions=e.successors.map(s=>{
      const raw=c?.players?.[String(s.playerId)];if(!raw)return 'Spielerzuordnung unbekannt';
      const p=sleeperPlayerRow(s.playerId,c.players);
      if(s.availability==='OWNED')return p.name+' · Roster/Trade-Auswirkung prüfen; kein FA-Add';
      const decisions=(c.seasonRows||[]).map(drop=>({drop,...seasonAcquisitionDecision(drop,{p},c.seasonRows,c.season)})).filter(x=>x.action!=='HOLD').sort((a,b)=>b.utility-a.utility);
      return decisions.length?decisions[0].action+': '+p.name+' / DROP '+decisions[0].drop.p.name+' · '+decisions[0].reason:p.name+' · WATCH; Add/Drop-Evidence nicht ausreichend';
    });
    return '<div class="coach-row"><div><b>'+esc(e.status)+'</b><div class="tiny">Spieler '+esc(e.playerId)+' · '+esc(actions.join(' / ')||'Rollenfolge nicht verifiziert')+'</div><div class="tiny">'+esc(e.invalidator)+'</div></div></div>';
  }).join(''):'<div class="notice">Keine verifizierte aktuelle News-Aktivierung. Research-Seeds sind keine Handlungsempfehlung.</div>';
}
...
function evidenceIdentity(e){return [e.sourceId||'',e.playerId||e.playerKey||'',e.evidenceType||'',e.sourcePublishedAt||e.observedAt||'',e.thesisPath||'',...(e.eventKey?[e.eventKey,e.status,e.verified,e.verifiedAt,e.conflict]:[])].join('|')}
function appendResearchEvidence(input){
  ...
  Object.assign(e,{eventKey:String(input.eventKey||''),status:String(input.status||'UNKNOWN'),sourceUrl:String(input.sourceUrl||''),sourceTier:Number.isInteger(input.sourceTier)?input.sourceTier:null,verified:input.verified===true,verifiedAt:Number(input.verifiedAt||0),conflict:input.conflict===true});
...
async function syncWatcherFeed(){
  ...
  try{
    ...
    const gate=String(v?.gate?.overall||'UNKNOWN'),isV2=v.schema==='draft-companion.watcher-feed.v2';
    const market=String(v?.gate?.market||'UNKNOWN'),playerState=String(v?.gate?.player_state_status||'UNKNOWN');
    const healthyMarket=!isV2||market==='PASS',healthyPlayerState=!isV2||playerState==='PASS';
    if(isV2&&gate==='PASS'&&!healthyMarket&&!healthyPlayerState)throw new Error('Watcher v2 Gate inkonsistent: overall PASS ohne gesunde Sub-Lane');
    if(gate!=='PASS'){
    ...
    for(const row of Array.isArray(v.events)?v.events:[]){
      const eventLane=String(row?.fundamental_or_market||'fundamental');
      if(isV2&&((eventLane==='market'&&!healthyMarket)||(eventLane!=='market'&&!healthyPlayerState)))continue;
...
els.diagnoseBtn.onclick=async()=>{els.diagnostic.textContent='Teste Projektionen, aktuelle Expert-Ranks und Spielkontext …';els.diagnosticCopyBtn.disabled=true;try{const report=await runPhysicalEvidenceLaneDiagnostic();els.diagnostic.textContent=formatPhysicalEvidenceLaneDiagnostic(report);els.diagnosticCopyBtn.disabled=false}catch{els.diagnostic.textContent='Diagnose konnte nicht vollständig initialisiert werden. Keine Geheimnisse wurden ausgegeben.'}};
els.diagnosticCopyBtn.onclick=async()=>{const text=els.diagnostic.textContent;if(!text||els.diagnosticCopyBtn.disabled)return;try{await navigator.clipboard.writeText(text);els.diagnosticCopyBtn.textContent='Diagnose kopiert ✓';setTimeout(()=>els.diagnosticCopyBtn.textContent='Diagnose kopieren',1200)}catch{els.diagnosticCopyBtn.textContent='Markiere Bericht unten';els.diagnostic.setAttribute('tabindex','0');els.diagnostic.focus();const selection=globalThis.getSelection?.();selection?.removeAllRanges();const range=document.createRange();range.selectNodeContents(els.diagnostic);selection?.addRange(range)}};
...
function renderSeasonRankingFreshness(note=''){
  ...
  const api=globalThis.PittiWeeklyEvidenceV2,snapshot=store.get(api?.CACHE_KEY||'pitti.weekly-evidence.v2.current',null),now=Date.now(),validation=api?.validateSnapshot?.(snapshot,seasonEvidenceContext(),now)||{ok:false,reason:'WEEKLY_EVIDENCE_MODULE_MISSING'},valid=validation.ok===true,t=valid?Number(snapshot.lastSuccessAt||0):0,age=t?now-t:Infinity;
  const coverage=valid?seasonProjectionCoverage(snapshot):seasonProjectionCoverage(null),refreshFailed=/fehlgeschlagen|fail/i.test(note),retained=refreshFailed&&valid;
  ...
  if(els.seasonRankingStatus&&!seasonRankingRefreshBusy){
    const base=!valid?(refreshFailed?`${note} · kein gültiger aktueller Projection-Snapshot verfügbar`:`Keine gültige aktuelle Weekly Evidence (${validation.reason}). Automatische Prüfung wird versucht.`):retained?`${note} · letzter verifizierter gültiger Snapshot wird unverändert beibehalten`:(note||(age>SEASON_RANKING_AUTO_MS?'Weekly Evidence älter als 3 Std. · automatische Prüfung wird versucht.':coverage.complete?'FantasyPros Weekly Projections verifiziert · Expert-Ranks/Matchup-Lanes bleiben separat fail-closed.':'Weekly Projections nur teilweise aktualisiert · nicht verfügbare Positionen können unveränderte ältere Evidenz enthalten.'));
...
try{
  ...
  // renderAll/updateStatus touch shared draft UI and may replace Season status text.
  // Re-assert the physical execution marker immediately before any Season network work.
  if(els.seasonLiveStateAge)els.seasonLiveStateAge.textContent='JS gestartet';
  if(els.seasonLiveStateStatus){els.seasonLiveStateStatus.className='notice';els.seasonLiveStateStatus.textContent='Season-Modul gestartet · Live-Kader wird vorbereitet …';}
  if(els.rosterStatus)els.rosterStatus.textContent='Season-Modul gestartet · Live-Kader wird vorbereitet …';
  void (async()=>{
