...
const ids=['onlineState','rankingAge','adpCount','qualityMini','apiQuickStatus','qualityStatus','loadExpertsBtn','applyPresetBtn','loadAllRanksBtn','refreshAllBtn','presetStatus','panelStatus','adpFile','adpStatus','draftInput','slot','topN','snapshotMode','refreshBtn','copyBtn','shareBtn','autoRefresh','draftStatus','draftSummary','teamSummary','favoritesBlock','coachList','snapshot','emptyCoach','logDecisionBtn','clearLogBtn','decisionLog','apiKey','toggleKeyBtn','clearKeyBtn','season','scoring','activePanel','diagnoseBtn','diagnostic','expertSearch','expertsList','savePanelBtn','newPanelBtn','renamePanelBtn','deletePanelBtn','qbPanel','rbPanel','wrPanel','tePanel','backupBtn','restoreFile','clearDraftDataBtn'];
...
function scoreCandidate(p,current,next,state,available){const r=rankFor(p.name,p.pos),a=Number(adp[norm(p.name)]);if(!r)return{score:-999,r:null,a,reasons:['Panel-Rang fehlt']};let score=100-clamp((r.rank-1)*.65,0,70),reasons=[];if(Number.isFinite(a)){const value=current-a;score+=clamp(value*.45,-12,14);if(value>=8)reasons.push(`Value +${Math.round(value)}`);else if(value<=-12)reasons.push(`Reach ${Math.abs(Math.round(value))}`)}else reasons.push('ADP fehlt');score+=state.need[p.pos]||0;if((state.need[p.pos]||0)>=7)reasons.push(`${p.pos}-Need`);const sameTier=available.filter(x=>x.pos===p.pos&&rankFor(x.name,x.pos)?.tier===r.tier).length;if(sameTier<=2){score+=5;reasons.push(`Tier fast leer (${sameTier})`)}const agree=agreement(r.sd,r.n);if(agree==='Sehr hoher Konsens')score+=4;else if(agree==='Stark umstritten')score-=4;if(p.injury){score-=10;reasons.push(`Injury ${p.injury}`)}if(p.bye&&(state.byes[p.pos]?.[p.bye]||0)>=2){score-=1;reasons.push(`Bye ${p.bye} (Tiebreaker)`)}const ret=returnChance(next,a);if(ret!=null&&ret>.8)reasons.push('Sehr hohe Return-Chance');if(ret!=null&&ret<.25)reasons.push('Kaum Return-Chance');const confidence=clamp(Math.round(92-r.sd*2+(Number.isFinite(a)?4:-10)+(r.n>=3?4:0)),35,98);return{score:Math.round(clamp(score,0,100)),r,a,ret,reasons,agree,sameTier,confidence}}
function expertRanksHtml(r){return `<div class="expert-grid">${r.individual.map(x=>`<div class="expert-rank"><b>${esc(x.expertName)}</b><span>#${Number(x.rank).toFixed(0)}</span></div>`).join('')}</div>`}
function renderCoach(rows,state,current,next){const best=rows[0]?.score??0,favorites=rows.filter(x=>best-x.score<=3).slice(0,4);els.favoritesBlock.innerHTML=favorites.length?`<div class="favorite-box"><b>Favoriten für diesen Pick</b>${favorites.map(x=>`${esc(x.p.name)} (${x.p.pos}, ${x.score})`).join(' · ')}</div>`:'';els.coachList.innerHTML=rows.slice(0,8).map((x,i)=>`<article class="coach"><div class="coach-head"><div><h3>${i+1}. ${esc(x.p.name)} · ${x.p.pos}</h3><div class="tiny">${esc(x.r.panel)} · Tier ${x.r.tier||'–'} · ${esc(x.agree)}${x.p.bye?` · Bye ${x.p.bye}`:''}</div></div><div class="score">${x.score}</div></div><div class="metrics"><div class="metric"><b>${x.r.rank.toFixed(1)}</b><span>Overall</span></div><div class="metric"><b>${Number.isFinite(x.r.posRank)?x.r.posRank.toFixed(1):'–'}</b><span>${x.p.pos}-Rang</span></div><div class="metric"><b>${Number.isFinite(x.a)?x.a.toFixed(1):'–'}</b><span>ADP</span></div><div class="metric"><b>${x.ret!=null?Math.round(x.ret*100)+'%':'–'}</b><span>Return</span></div><div class="metric"><b>${x.confidence}%</b><span>Confidence</span></div></div>${expertRanksHtml(x.r)}<div class="tags">${x.reasons.map(r=>`<span class="tag ${r.startsWith('Reach')||r.startsWith('Injury')?'bad':r.includes('Return-Chance')?'warn':'ok'}">${esc(r)}</span>`).join('')}</div></article>`).join('');els.teamSummary.innerHTML=Object.entries(state.counts).map(([p,n])=>`<div class="summary-item"><b>${n}</b><span>${p}</span></div>`).join('')+`<div class="summary-item"><b>${current}</b><span>Pick</span></div><div class="summary-item"><b>${next??'–'}</b><span>Nächster</span></div>`}

async function refresh(){persist();const id=draftId(els.draftInput.value);if(!id)throw new Error('Draft-ID fehlt.');els.refreshBtn.disabled=true;els.draftStatus.textContent='Lade Draft …';try{const{draft,picks,players}=await fetchDraft(id),teams=Number(draft.settings?.teams||10),rounds=Number(draft.settings?.rounds||15),slot=Number(els.slot.value),total=teams*rounds,current=Math.min(picks.length+1,total),next=nextOwn(current,teams,slot,total),mine=picks.filter(p=>Number(p.draft_slot)===slot).sort((a,b)=>a.pick_no-b.pick_no),drafted=new Set(picks.map(p=>String(p.player_id)));const available=Object.entries(players).filter(([pid,p])=>!drafted.has(pid)&&['QB','RB','WR','TE'].includes(p.position)&&p.active!==false&&p.full_name).map(([pid,p])=>({id:pid,name:p.full_name,pos:p.position,team:p.team||'FA',searchRank:Number(p.search_rank),injury:p.injury_status||null,bye:p.bye_week||null})).sort((a,b)=>(a.searchRank||9999)-(b.searchRank||9999)).slice(0,Number(els.topN.value));const state=rosterState(mine,players),scored=available.map(p=>({p,...scoreCandidate(p,current,next,state,available)})).filter(x=>x.r).sort((a,b)=>b.score-a.score);renderCoach(scored,state,current,next);const best=scored[0]?.score??0,favorites=scored.filter(x=>best-x.score<=3).slice(0,4),lines=['===== SLEEPER DRAFT SNAPSHOT =====',`Draft-ID: ${id}`,`Status: ${draft.status}`,`Teams: ${teams} | Runden: ${rounds} | Mein Slot: ${slot}`,`Aktueller Pick: ${current}`,`Mein nächster Pick: ${next??'keiner'} | Picks bis dahin: ${next==null?'–':next-current}`,'','DATENSTATUS',`Panel-Rankings: ${Object.keys(panelRanks).length} | Sleeper-ADP: ${Object.keys(adp).length}`,'','MEIN TEAM'];for(const pick of mine){const p=pinfo(String(pick.player_id),pick.metadata,players);lines.push(`${pick.pick_no}. ${p.name} — ${p.pos}, ${p.team}`)}if(!mine.length)lines.push('Noch keine Picks.');lines.push('','FAVORITEN');favorites.forEach((x,i)=>lines.push(`${i+1}. ${x.p.name} — ${x.p.pos}, ${x.p.team} | Coach ${x.score} | Panel ${x.r.rank.toFixed(1)} | ADP ${Number.isFinite(x.a)?x.a.toFixed(1):'FEHLT'} | Return ${x.ret!=null?Math.round(x.ret*100)+'%':'FEHLT'} | Confidence ${x.confidence}%`));lines.push('','DRAFT COACH TOP 8');scored.slice(0,8).forEach((x,i)=>{lines.push(`${i+1}. ${x.p.name} — ${x.p.pos}, ${x.p.team} | Coach ${x.score} | ${x.r.panel} ${x.r.rank.toFixed(1)} Tier ${x.r.tier||'–'} | ADP ${Number.isFinite(x.a)?x.a.toFixed(1):'FEHLT'} | Return ${x.ret!=null?Math.round(x.ret*100)+'%':'FEHLT'} | Confidence ${x.confidence}% | ${x.agree}`);lines.push(`   Einzelrankings: ${x.r.individual.map(v=>`${v.expertName} #${Math.round(v.rank)}${Number.isFinite(v.posRank)?` (${x.p.pos}${Math.round(v.posRank)})`:''}`).join(' · ')}`)});lines.push('','HARTE REGEL','Kein großer Reach ohne konkrete aktuelle Begründung. Fehlende Panel- oder ADP-Daten ausdrücklich als Unsicherheit behandeln. K und DST werden nicht gedraftet. Bye Weeks sind nur ein kleiner Tiebreaker.','','AUFGABE','Prüfe aktuelle Verletzungen, Depth Charts und News. Nenne einen oder mehrere gleichwertige Favoriten, 2–3 Alternativen, Return-Chancen und Confidence. Erzwinge keine Einzelentscheidung, wenn mehrere Spieler nahezu gleichauf liegen. Abweichungen vom Expertenpanel oder der Sleeper-ADP ausdrücklich begründen.');els.snapshot.value=lines.join('\n');els.draftStatus.className='notice ok';els.draftStatus.textContent=`${picks.length} Picks geladen · ${scored.length} Kandidaten bewertet.`;els.draftSummary.hidden=false;els.emptyCoach.hidden=true;els.copyBtn.disabled=false;els.shareBtn.disabled=false;lastDraftContext={id,current,next,favorites,scored,picks,mine};}finally{els.refreshBtn.disabled=false}}
...
function backup(){return{format:'draft-companion-v7',version:'7.2.0',createdAt:new Date().toISOString(),season:els.season.value,scoring:els.scoring.value,experts,panels,activePanelId,positionPanels,rankCache,panelRanks,adp,adpMeta,decisionLog,draft:els.draftInput.value,slot:els.slot.value}}
