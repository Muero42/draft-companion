...
function freezeDecisionFixture({draftId,current,returnPick,picks,mine,rankedAvailable,scored,rv2,mode,strategy,stress,teams,slot,fingerprint}){
  ...
  rows.push({
    ...
    modelVersion:'v11.8.0-rc4.17',researchResidualModel:RESEARCH_RESIDUAL_MODEL_VERSION,managerProfileHash:MANAGER_PROFILE_SOURCE_HASH,rng:{runs:rv2?.runs??900,seedBasis:`${current}|${returnPick??'end'}|${stress}`},
...
/*
 * rc4.17 — Research Residual Shadow v2.
 *
...
async function refresh(){
  ...
  try{
    ...
    // the live Coach score in rc4.17. This preserves a clean prospective baseline.
    ...
    const lines=[
      ...
      `Coach-Modell: v11.8.0-rc4.17 Return-v2 · Strategie ${strategyLabel(strategy)} · Modus ${mode} · Stress ${stressLabel(stress)} · Panel-first · Return + Gegnerroster + plausible Abnehmer${managerProfilesActive(mode,els.season.value,teams)?' + Manager-Layer':''} · Loss-if-Gone`,
