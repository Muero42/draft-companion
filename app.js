...
function pittiDecisionAuditRow(x,c){if(!x)return null;return{name:x.p.name,pos:x.p.pos,team:x.p.team,injury:x.p.injury||null,panel:x.r.rank,panelId:x.r.panelId||null,panelName:x.r.panel||null,expertCount:Number.isFinite(Number(x.r.n))?Number(x.r.n):null,panelSd:Number.isFinite(Number(x.r.sd))?Number(x.r.sd):null,individual:x.r.individual||[],adp:Number.isFinite(x.a)?x.a:null,ret:x.ret,returnConfidence:x.returnConfidence,confidence:x.confidence,score:x.score,rawScore:x.rawScore,action:x.action,loss:x.loss,outsideNormalCut:!normalCandidateAdmissible(x),role:pittiFantasyRole(x.p,c.players),arrows:pittiResearchArrows(x),reasons:Array.isArray(x.reasons)?x.reasons.slice():[],researchResidual:x.researchResidual||null,valueSafety:x.valueSafety||null,nearAlternatives:x.nearAlternatives??null,alternativeGap:x.alternativeGap??null,valueKind:x.valueKind??null};}
...
const RESEARCH_RESIDUAL_PRIORS={
  ...
  [norm('Kenneth Walker III')]:{pos:'RB',expiresAt:Date.parse('2026-09-02T12:00:00Z'),components:[{kind:'elite_efficiency_role',dir:0,strength:1,confidence:.94,display:true,causal:'2025 trotz Seattle-Nutzung #2 Explosive-Run-Rate und #1 Missed-Tackle-Rate; Kansas City gibt ihm einen klareren Lead-/Red-Zone-Pfad als 2025',invalidator:'Kansas City bleibt echte Committee-Nutzung oder Effizienz übersetzt nicht'},{kind:'current_foot',dir:0,strength:1,confidence:.94,displayRisk:true,causal:'Aktuelle Fußverletzung Ende August ist ein Draft-Freshness-Gate; Week-1-Verfügbarkeit und Belastung müssen vor dem Pick bestätigt sein',invalidator:'volle Trainingsfreigabe ohne Einschränkung'}]},
  ...
  [norm('Sam LaPorta')]:{pos:'TE',expiresAt:Date.parse('2026-09-02T12:00:00Z'),components:[{kind:'efficient_te1',dir:0,strength:1,confidence:.93,display:true,causal:'Vor dem 2025-Rücken-Aus war LaPorta TE7 in Punkten/Spiel und blieb per Route effizient; Petzing-System bietet weiterhin Top-8- mit Top-5-Ceiling',invalidator:'Target-Rate bleibt dauerhaft niedrig oder Effizienz fällt'},{kind:'current_health_target_volume',dir:0,strength:1,confidence:.94,displayRisk:true,causal:'Nach Rückenverletzung kommt nun ein Camp-Hüftproblem hinzu; zusätzlich lag sein Target-Volumen zwei Jahre klar unter der Rookie-Saison',invalidator:'volle Gesundheit plus deutlicher Target-Anstieg'}]},
