...
function decisionEvidenceExport(){
  ...
  return{format:'pitti-decision-evidence-v1',appVersion:APP_VERSION,createdAt:new Date().toISOString(),draftId:id||null,slot:Number(els.slot.value),mode:els.draftMode.value,fixtureCount:rows.length,resolvedCount:rows.filter(f=>f.chosenPlayer).length,overrideCount:rows.filter(f=>f.decisionOutcome&&f.decisionOutcome.followedCoach===false).length,fixtures:rows};
}
