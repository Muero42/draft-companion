...
function renderPanelSummary(){
  ...
  const embedded=[...new Set(positions.flatMap(pos=>embeddedPanelExpertNames(positionPanels[pos])))];
  const activeEmbedded=embedded.length>0;
...
function panelSelectable(id){
  ...
  if(/^expert-v2-(qb|rb|wr|te)$/.test(id))return true;
...
function freezeDecisionFixture({draftId,current,returnPick,picks,mine,rankedAvailable,scored,rv2,mode,strategy,stress,teams,slot,fingerprint,map}){
  ...
  rows.push({
    ...
    modelVersion:'v11.8.0-rc4.80',researchResidualModel:RESEARCH_RESIDUAL_MODEL_VERSION,managerProfileHash:MANAGER_PROFILE_SOURCE_HASH,managerMapSnapshot:{...(map||{})},managerLiveStateSnapshot:JSON.parse(JSON.stringify(LIVE_MANAGER_ADAPTATION_STATE)),rng:{runs:rv2?.runs??900,seedBasis:`${current}|${returnPick??'end'}|${stress}`},
...
const RESEARCH_RESIDUAL_PRIORS={
  ...
  [norm('Christian McCaffrey')]:{pos:'RB',expiresAt:Date.parse('2026-09-02T12:00:00Z'),components:[{kind:'decline_tail',dir:-1,strength:.55,confidence:.66,pricing:'LIKELY_PRICED',causal:'age + extreme recent workload + durability interaction',invalidator:'healthy camp + maintained efficiency/role'},{kind:'current_camp_context',dir:0,strength:1,confidence:.9,displayRisk:true,causal:'Camp-Tightness zuletzt gemanagt; seit 24.8 wieder in Team-Drills, Auszeit laut McCaffrey Teil des Plans',invalidator:'erneute Trainingspause oder Belastungsrückschritt'}]},
