# Codex handoff audit v263

Generation: `20260923T1834Z-v263`

Live authority was freshly verified for `Muero42/draft-companion`: canonical `main` is `d5954d66877df877f950a4a41f32baad59a66748`; PR #191 is merged; PR #192 remains Draft, targets `main`, and has exact head `6c523297` with stale v262 authority text. PR #192 must not be merged unchanged.

The v262 claim that rc4.205 remained Production authority is superseded. `v11.8.0-rc4.206` is both source and Production authority via deployment `8322e9b3-a293-4454-8ef9-d5e98c217889`, but its device acceptance is false under `RC4.206_PHYSICAL_PARTIAL_PASS_NOT_ACCEPTED_SELECTED_PANEL_AND_GAME_CONTEXT_UNAVAILABLE`. rc4.205 remains only the last device-accepted control.

The rc4.207 repair is bounded to current-week selected-expert identity acquisition and one official ESPN CDN schedule fallback. It preserves projection, Sleeper hydration, Start/Sit, broad-ECR separation, complete-week uniqueness validation, sanitized errors, and fail-closed semantics. Team Total remains unavailable. No merge, Production deployment, Watcher mutation, external communication change, or Sleeper/fantasy transaction is part of this checkpoint.

Next safe gate: `RC4207_EXACT_HEAD_CI_AND_NONPRODUCTION_PREVIEW` — exact-head local/CI validation and a non-Production Cloudflare preview for the repair PR.
