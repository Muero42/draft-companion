...
Stages: isolated protection inspection → trusted preflight → official Codex implementation → schema-bound patch → fresh-runner validation → separate publisher → fresh exact-head CI → separate read-only Codex review → receipt awaiting explicit user approval. Every job checks out its controller from `github.workflow_sha`, the trusted main workflow revision, before validating the user-supplied expected SHA; candidate code cannot replace the publisher. Requests are re-bound to GitHub's workflow inputs/run/attempt rather than trusted from agent-writable artifacts. Expected main is checked again before publication, CI and review.

The repository has one human promotion principal. GitHub therefore requires the PR and exact-head checks but zero human approvals; Code Owner review and most-recent-push approval are disabled as merge gates. Independence comes from the separate read-only Codex review and its immutable receipt, not from a second identity controlled by the same owner. Promotion still requires a fresh explicit `Muero42` approval for the exact PR/head/main through the fail-closed promotion contract. The App cannot call merge or bypass main protection.

The initial budget is deliberately **one first attempt**, one 25-minute implementation invocation and one 25-minute review invocation; jobs also have deadlines. A bounded agent can diagnose and repair within that invocation. Failed tests publish nothing. Failed later CI/review leaves an unmerged work PR and a failed run; workflow reruns are rejected. A new explicitly dispatched task creates a new unique branch. No endless retries, implicit budget reset or automatic merge exists. This Foundation does not claim a monetary spend cap; configure the API project's budget separately.

Workbranch: `pitti/cloud-auto/<task_id>-<run_id>-1`. Existing remote branches are never overwritten. A failed run requires a new explicit dispatch and therefore a new run ID. The Foundation authoring branch `pitti/cloud-auto-foundation` is separate.
...
Codex does not commit/push. It is the final explicit step of its job and returns only a schema-constrained, canonical base64 Git patch (maximum 128 KiB) plus summary through the action output. A fresh job applies that patch. The trusted publisher creates exactly one commit from the validated tree through GitHub's Git database API and atomically creates a previously absent ref; it never invokes Git push or any hook.

Candidate tests run under Bubblewrap with unshared network/PID/IPC/UTS/cgroup/mount namespaces, an allowlisted environment, no home or runner credential mounts, read-only candidate/controller/browser mounts and a dedicated writable output. Ubuntu 24 launches the trusted namespace wrapper through sudo because unprivileged user namespaces are restricted; After mounting, setpriv drops the candidate to UID/GID 65534 with empty capability sets and no-new-privs. The host wrapper rejects links and writes the authoritative receipt only after the namespace exits. The real namespace-denial probe runs on every Linux validation host. Before publication it rechecks base, branch, patch hash, tree, scope, clean tracked content, server protection and obvious secret-material signatures. This scan is defense in depth, not proof that arbitrary text cannot contain a secret. Credentials are never provided to candidate tests or the implementation process.
...
6. GitHub must permit the workflow's read token to inspect Actions and PRs. A separate pre-agent job uses a short-lived App token to inspect full rulesets, then revokes it before implementation starts. The publisher repeats that inspection. GitHub omits bypass lists without ruleset-write access: an omitted list is UNKNOWN, never an empty list. Ruleset history and version endpoints also require Administration: write and are forbidden to this App, so stored IDs/version pins cannot close that evidence gap. Under the current GitHub API boundary the protection job is expected to stop before implementation and publication. This Foundation remains fail-closed unless GitHub later exposes sufficient read-only proof; do not widen App Administration/main permissions or substitute a boolean, pin, or owner assertion claiming protection exists.
...
Existing three workflows retain their checks and add Foundation regressions; all executable third-party actions are immutable commit pins and all checkouts disable credential persistence. checkout explicitly targets the PR head instead of silently using a synthetic merge checkout. `PITTI cloud validation` adds the unique job `pitti-cloud-validation`: full Strict Suite, YAML contracts, mobile Chromium, package/re-extraction, behavioral tests on re-extracted bytes, seal and no tracked mutations. The takeover validator consumes freshly collected GitHub evidence after all four CI workflows pass. A new head or changed main invalidates the old review/CI basis.

Review runs on a separate runner/context with no code-write permission. It receives the complete diff, exact head, current main, CI and takeover evidence; checks 14 distinct semantic topics and adversarial counterexamples. The Codex Action is the final step in that job and returns only schema-bound JSON. A fresh trusted job reconstructs the immutable diff binding, rechecks GitHub state, and creates the receipt. The receipt requires exact SHAs, no unresolved findings, distinct evidenced topics, independent=true and readOnly=true. A failed/incomplete review cannot produce a PASS receipt.
