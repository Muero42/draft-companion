...
The API restricts `bypass_actors` visibility to callers that can write rulesets and also redacts the detail timestamp from the least-privilege response. The publisher App must not receive that Administration capability. The controller reads the Metadata-accessible ruleset history and accepts an omitted bypass list only when the ruleset name, numeric ID and latest immutable `version_id` match the owner-verified pins in `tools/cloud-contract.mjs`; every ruleset edit creates a new version and fails closed. When GitHub returns `bypass_actors`, the live value is always validated directly. Updating a pin requires a separately reviewed Foundation maintenance change backed by a fresh privileged read. Never grant the App Administration or main bypass to make inspection pass.
