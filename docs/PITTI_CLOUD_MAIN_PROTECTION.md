...
GitHub restricts `bypass_actors`, ruleset history, and ruleset versions to callers with **Administration: write**. The publisher App must never receive that capability. Numeric IDs, names, rule bodies, timestamps, or previously observed version IDs cannot prove that a redacted bypass list is unchanged. The controller therefore uses only the Metadata-readable list/detail endpoints and treats any omitted `bypass_actors` value as UNKNOWN, which stops protection inspection and all dependent publication. It never calls history/version endpoints and never substitutes a stored pin. This deliberately leaves Cloud AUTO publication blocked under the current API boundary; implementation and publication are not allowed to proceed on unverifiable main protection. Do not grant the App Administration or main bypass to make inspection pass.

An owner attestation bound to list/detail `id`, `updated_at`, and every other visible field does not close the gap. GitHub does not document `updated_at` as a security invariant that must change for every bypass edit, so an equal timestamp cannot prove an equal redacted list. Delete/recreate changes identity but supplies no bypass proof; rollback can restore the visible body without proving the hidden body; an additional targeting ruleset can add a bypass; and configuration can change between inspection and ref creation (TOCTOU). Rule-suite results describe rule evaluation for a ref and are not an authenticated, complete snapshot of every configured bypass actor. Consequently there is no supported stronger live-evidence path at this App's present permission boundary.

The irreducible gate is a current privileged human/owner inspection of all targeting rulesets and their complete bypass lists, followed by manual promotion outside Cloud AUTO. That observation is audit evidence only: it must not be converted into a reusable pin or attestation that lets the Metadata-only controller publish. Automated publication remains blocked unless GitHub provides a read-only, complete, freshness-bound representation of bypass configuration.
