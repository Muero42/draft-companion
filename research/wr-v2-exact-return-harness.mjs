...
const INPUT='research/expert-v2-exact-return-input-v2.json.gz.b64',APP='app.js';
const data=JSON.parse(zlib.gunzipSync(Buffer.from(fs.readFileSync(INPUT,'utf8').replace(/\s+/g,''),'base64')).toString('utf8'));
const app=fs.readFileSync(APP,'utf8');
if(data.kernelCommit!=='9ba6db89fc1e7550052a7526bd0c68d6cc7459dc')throw Error('kernel pin drift');
if(data.schema!=='pitti-expert-v2-exact-return-input-v2')throw Error('input schema drift');
const start=app.indexOf('const MANAGER_PROFILES='),end=app.indexOf('function returnValidationKey()');
...
const els={season:{value:'2026'}};const localStorage={getItem:()=>null,setItem:()=>{},removeItem:()=>{}};
...
function rankFor(name,pos){const r=globalThis.__rankMap[norm(name)];return Number.isFinite(r)?{rank:r}:null}
...
for(const f of data.fixtures){if(!Number.isFinite(f.next))continue;const run=runFixture(f,'control');for(const[key,expected]of Object.entries(f.capturedReturn||{})){const z=resolvedReturn(f,'control',key,run);if(z.kind==='rv2')rv2N++;else fallbackN++;const e=Math.abs(z.ret-+expected);n++;sum+=e;max=Math.max(max,e)}}
console.log(`CONTROL_PARITY predictions=${n} rv2=${rv2N} fallback=${fallbackN} mae=${n?sum/n:NaN} max=${max}`);if(max!==0)process.exit(2);
for(const f of data.fixtures){if(!Number.isFinite(f.next)||f.current<69)continue;const c=runFixture(f,'control'),w=runFixture(f,'wr-v2'),out={};for(const key of f.available){const a=resolvedReturn(f,'control',key,c),b=resolvedReturn(f,'wr-v2',key,w);out[key]={c:a.ret,w:b.ret,ck:a.kind,wk:b.kind};}console.log('RESOLVED_RETURNS='+JSON.stringify({pick:f.current,next:f.next,returns:out}));}
