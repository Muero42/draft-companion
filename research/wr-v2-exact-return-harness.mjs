...
console.log('WR_ONLY_RETURN_DIFFS');for(const f of data.fixtures){if(!Number.isFinite(f.next))continue;const c=runFixture(f,'control'),w=runFixture(f,'wr-v2'),names=new Set([...Object.keys(c.rv.players),...Object.keys(w.rv.players)]);const diffs=[...names].map(k=>({k,c:c.rv.players[k]?.ret,w:w.rv.players[k]?.ret})).filter(x=>Number.isFinite(x.c)&&Number.isFinite(x.w)&&Math.abs(x.c-x.w)>1e-12).sort((a,b)=>Math.abs(b.w-b.c)-Math.abs(a.w-a.c));console.log(JSON.stringify({pick:f.current,roster:f.rosters[String(data.slot)],maxReturnDelta:diffs.length?Math.max(...diffs.map(x=>Math.abs(x.w-x.c))):0,changedTop24:[...names].filter(k=>!(k in c.rv.players)||!(k in w.rv.players)),largest:diffs.slice(0,8)}))}
