'use strict';
/* Research-only roster-reality challenger.
   Preserve metadata-safe rc4.59 Coach/opponent/seed logic and existing soft QB2/TE2 scoring.
   Repeat QB/TE stays on the full board and can still win naturally; it is safety-eligible only
   if already the natural pre-safety leader or if the existing elite-slide exception is met.
   Draft-end feasibility matches observed user/platform behavior: TE0 is legal and may be
   resolved by pre-Week-1 streaming/FA. No app/runtime/production mutation. */
...
if(full0.split(safetyAnchor).length!==2)throw Error('SAFETY_ANCHOR_DRIFT');
const safetyPatch=`const preSafetyValid=scored.filter(x=>x?.r&&Number.isFinite(x.r.rank)&&Number.isFinite(x.rawScore)&&!x.hardExcluded&&!x.recommendationBlocked);const naturalPreSafety=preSafetyValid.slice().sort((a,b)=>b.rawScore-a.rawScore||a.r.rank-b.r.rank)[0];const safetyRows=scored.filter(x=>{const pos=x.p.pos,n=Number(state.counts?.[pos]||0);if((pos==='QB'||pos==='TE')&&n>=1){const elite=pos==='QB'?(x.r.rank<=45&&Number.isFinite(x.a)&&pn-x.a>=35):(x.r.rank<=35&&Number.isFinite(x.a)&&pn-x.a>=30);return x===naturalPreSafety||elite}return true});const safety=C.applyPlayerQualitySafetyGate(safetyRows,pn);`;
let full=full0.replace(safetyAnchor,safetyPatch);
const feasibilityAnchor="H.ok(c.QB>=1&&c.RB>=1&&c.WR>=2&&c.TE>=1,'starter feasibility');";
if(full.split(feasibilityAnchor).length!==2)throw Error('FEASIBILITY_ANCHOR_DRIFT');
full=full.replace(feasibilityAnchor,"H.ok(c.QB>=1&&c.RB>=1&&c.WR>=2,'draft-end user-policy feasibility');");
