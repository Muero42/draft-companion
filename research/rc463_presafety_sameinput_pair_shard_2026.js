...
/* Research-only same-input parity harness.
   Root cause addressed: the old parity comparison paired runs that independently fetched mutable
   Sleeper player metadata. Both arms now read one exact workflow snapshot from disk. This also
   avoids the counterfactual harness's intentional fake-fetch guard. No runtime mutation. */
...
function patched(src,label){
 const re=/const res=await fetch\('https:\/\/api\.sleeper\.app\/v1\/players\/nfl',\{headers:\{'User-Agent':'PITTI-FullSafety(?:Baseline|PreSafety)\/1\.0'\}\}\);if\(!res\.ok\)throw Error\('metadata fetch '\+res\.status\);const j=await res\.json\(\);/g;
 const hits=src.match(re)||[];if(hits.length!==1)throw Error(label+'_METADATA_ANCHOR_DRIFT '+hits.length);return src.replace(re,shared)
}
...
const metaPath=process.env.PITTI_META_SNAPSHOT||'/tmp/meta/players.json',meta=JSON.parse(fs.readFileSync(metaPath,'utf8'));
const pw=Object.entries(meta).find(([,p])=>String(p?.full_name||'').toLowerCase()==='parker washington');
const out={schema:1,status:'PASS',research_only:true,production_mutation:false,shard,seed_start:459820001+10*shard,seed_end:459820010+10*shard,metadata_snapshot_sha256:crypto.createHash('sha256').update(fs.readFileSync(metaPath)).digest('hex'),parker_washington_snapshot:pw?{sleeper_id:pw[0],injury_status:pw[1]?.injury_status??null,years_exp:pw[1]?.years_exp??null,team:pw[1]?.team??null}:null,parity:'10/10 exact decision-core + roster + fingerprint',rows};
