'use strict';
/* Research-only corrected hybrid challenger.
   IMPORTANT: rc4.59 coach helpers are lexical closures. Therefore this harness patches the
   app source BEFORE context() extracts rosterState/rosterExceptionPenalty/scoreCandidate.
   It also adds fail-closed canaries and a hard QB3/TE3 cap. No production mutation. */
...
/* 1) Metadata-safe mapping: normalized name + position, unique-match fail closed. */
...
/* 2) Patch lexical app source BEFORE context() extracts helper functions. */
const appLine="const app=fs.readFileSync('app.js','utf8');";
if(policy.split(appLine).length!==2)throw Error('APP_SOURCE_ANCHOR_DRIFT');
...
if(app.split(oldNeeds).length!==2)throw Error('LEXICAL_NEED_ANCHOR_DRIFT');
app=app.replace(oldNeeds,newNeeds);
...
/* Export mutated lexical helpers solely for canary verification. */
...
if(policy.split(exportTail).length!==2)throw Error('EXPORT_ANCHOR_DRIFT');
policy=policy.replace(exportTail,exportTail2);

/* Canary executes after context() but before any draft. It proves lexical mutation happened. */
const ctxAnchor='const C=context(), MAN=C.__MANAGER_PROFILE_DATA;';
if(policy.split(ctxAnchor).length!==2)throw Error('CONTEXT_ANCHOR_DRIFT');
...
const __rs=C.__rosterState([],{},129);if(__rs.need.QB!==13||__rs.need.TE!==1.5)throw Error('CANARY_NEED_EMPTY');`;
policy=policy.replace(ctxAnchor,canary);

/* 3) Effective admissibility layer in the full-mock caller: no QB3/TE3 ever; before pick
      121, QB2/TE2 only for the same absurd elite-slide exceptions. */
const scoreAnchor="let scored=ranked.map(p=>({p,...C.scoreCandidate(p,pn,next,state,ranked,'progressive')})).filter(x=>x.r),rv=null;";
if(full0.split(scoreAnchor).length!==2)throw Error('SCORE_ANCHOR_DRIFT');
...
const seedAnchor="const seeds=Array.from({length:60},(_,i)=>459710001+i),drafts=[];";if(full.split(seedAnchor).length!==2)throw Error('SEED_ANCHOR_DRIFT');
const start=459710001+10*shard;full=full.replace(seedAnchor,`const seeds=Array.from({length:10},(_,i)=>${start}+i),drafts=[];`);

fs.writeFileSync(POLICY,policy);const tmp=path.join('/tmp',`pitti_meta_safe_qbte_hybrid_v2_${shard}.js`);fs.writeFileSync(tmp,full);let status=2;
try{
  const r=cp.spawnSync(process.execPath,[tmp],{stdio:'inherit',env:process.env});if(r.error)throw r.error;status=r.status??2;
  if(status===0){
    const src='simulation_2026/RC459_REALISTIC_FULLMOCK_TIER_AUDIT_2026.json';if(!fs.existsSync(src))throw Error('OUTPUT_MISSING');
    const x=JSON.parse(fs.readFileSync(src,'utf8'));if(x.status!=='PASS'||x.seeds!==10||x.drafts?.length!==10)throw Error('OUTPUT_INVALID');
    const expected=Array.from({length:10},(_,i)=>start+i);if(JSON.stringify(x.drafts.map(d=>d.seed))!==JSON.stringify(expected))throw Error('SEED_MISMATCH');
    for(const d of x.drafts){const c=d.position_counts||{};if((c.QB||0)>2||(c.TE||0)>2)throw Error('CAP_INVARIANT '+d.seed+' '+JSON.stringify(c));}
    x.metadata_mapping='normalized-name + position, unique-match fail-closed';x.metadata_collision_source_bug_quarantined=true;
    x.qbte_repeat_policy='corrected hybrid v2: lexical rc4.60 phase penalties + pre-121 QB2/TE2 admissibility gate + hard QB3/TE3 cap';
    x.lexical_policy_canary='PASS';x.production_mutation=false;
    const dst=`simulation_2026/RC459_META_SAFE_QBTE_HYBRID_V2_SHARD_${shard}_2026.json`;fs.renameSync(src,dst);fs.writeFileSync(dst,JSON.stringify(x));
    console.log(JSON.stringify({status:'PASS',shard,start,end:start+9,lexical_policy_canary:'PASS',output:dst},null,2));
  }
}finally{fs.writeFileSync(POLICY,policy0)}
