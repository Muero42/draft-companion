...
// No NEW v3/v5 late-WR or broad >=1 raw-score hurdle may leak into this minimal candidate.
// Some generic strings (e.g. WR>=6) already exist elsewhere in frozen rc4.63, so compare exact occurrence counts.
for(const bad of ['__decisionPriorityV3','__decisionPriorityV5','late_wr_neartie','rawScore)+1.0','RB<7','WR>=6'])ok(count(cand,bad)===count(base,bad),`new bundled policy marker ${bad}: ${count(base,bad)} -> ${count(cand,bad)}`);
