...
Research-only: completed 2026 Sleeper mocks define the market; league-manager profiles are capped modifiers.
"""
...
def get_json(url):
 req=urllib.request.Request(url,headers={"User-Agent":"draft-companion-manager-mock-v2/0.5"})
 with urllib.request.urlopen(req,timeout=30) as r:return json.load(r)
def slot_at_pick(p):
 rnd=(p-1)//TEAMS+1;w=(p-1)%TEAMS+1;return w if rnd%2 else TEAMS-w+1
def own_picks():return[p for p in range(1,TEAMS*ROUNDS+1) if slot_at_pick(p)==USER_SLOT]
...
def collect(ids):
 ...
 for did in ids:
  try:
   ...
   if d.get("status")!="complete"or len(picks)<150:raise RuntimeError("incomplete")
   for pk in picks:
    if slot_at_pick(int(pk["pick_no"]))!=int(pk["draft_slot"]):raise RuntimeError("geometry")
   drafts[did]=picks
  except Exception as e:errors[did]=str(e)
 return drafts,errors
def build_empirical(drafts,include_user_tail=True):
 ...
 for picks in drafts.values():
  for pk in picks:
   ...
   if ds==USER_SLOT:continue
   opp[nm].append(pn);phase_pos[phase(pn)][po]+=1
 stats={n:PlayerStat(n,pos[n],v)for n,v in opp.items()}
...
def loo_metrics(drafts):
 ...
 for hold,picks in drafts.items():
  stats,_=build_empirical({k:v for k,v in drafts.items()if k!=hold},False);errs=[];covered=total=slides=0
  for pk in picks:
  ...
  out.append({"draft_id":hold,"coverage":covered/total,"mae":statistics.mean(errs),"n":len(errs),"extreme_slides":slides,"extreme_slide_rate_per_draft":slides})
 return out
def manager_mult(data,mgr,pos,ph,shrink=.30):
...
def calibration(stats,samples):
 ...
 for nm,st in stats.items():
  ...
  s=samples.get(nm,[])
  if len(s)<5:continue
  sm=statistics.mean(s);errs.append(abs(sm-st.mean));bias.append(sm-st.mean)
 return{"players":len(errs),"mean_abs_center_error":statistics.mean(errs),"mean_bias":statistics.mean(bias)}
def scenario(stats,data,runs,mode,use_manager,seed):
 samples,slides,invalid=simulate(stats,data,runs,seed,mode,use_manager);return{"runs":runs,"bjorn_mode":mode,"manager_layer":use_manager,"calibration":calibration(stats,samples),"extreme_slide_rate_per_draft":len(slides)/runs,"invalid_count":len(invalid),"invalid":invalid[:10]}
def main():
 ...
 stats,shares=build_empirical(drafts);data=load_profiles(Path(a.app));loo=loo_metrics(drafts);emp=statistics.mean(r["extreme_slide_rate_per_draft"]for r in loo)
 sc={"market_only":scenario(stats,data,a.runs,"auto",False,260817),"manager_bjorn_auto":scenario(stats,data,a.runs,"auto",True,260818),"manager_bjorn_manual_rb":scenario(stats,data,a.runs,"manual_rb",True,260819)}
 report={"schema":"draft-companion.manager-mock-v2.validation.v5","draft_ids":list(drafts),"errors":errors,"geometry":{"own_picks":own_picks()},"empirical":{"opponent_players":sum(s.source=="opponent"for s in stats.values()),"user_tail_players":sum(s.source=="user_tail"for s in stats.values()),"total_universe":len(stats),"phase_position_shares":shares,"loo_mean_extreme_slide_rate_per_draft":emp},"leave_one_mock_out":loo,"scenarios":sc,"policy":{"market_process":"sequential empirical hazard","manager_layer":"phase shares shrunk 30%; Bjorn auto bypasses profile","status":"RESEARCH_ONLY_NOT_LIVE"}}
