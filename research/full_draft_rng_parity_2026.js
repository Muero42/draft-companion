...
function special(r,c,pn){const hk=c.K===0&&pn>110?.18:0,hd=c.DEF===0&&pn>110?.16:0,anyp=1-(1-hk)*(1-hd);if(r()>=anyp)return null;if(hk<=0)return'DEF';if(hd<=0)return'K';return r()<hk/(hk+hd)?'K':'DEF'}
...
for(const seed of seeds){const a=simulate(legacyRng,seed),b=simulate(statefulRng,seed);assert.deepStrictEqual(b,a,'whole-draft mismatch seed '+seed)}
console.log(JSON.stringify({status:'PASS',gate:'whole-draft-legacy-stateful-parity',seeds,draw_shapes:['weighted-choice','special-multi-draw'],drafts:seeds.length,picks_per_draft:150,fingerprint:sha(seeds.map(s=>simulate(legacyRng,s).picks))}));
