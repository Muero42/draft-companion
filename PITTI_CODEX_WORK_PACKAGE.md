...
- Trade values must share sourceId, sourceUrl and chart publication timestamp; a provider name alone does not make different charts comparable. Active roster validation includes K/DST and excludes actual Reserve/TAXI membership. Capacity uses the complete live roster on both sides.
...
- eventKey identifies one real event across reports; it must distinguish separate occurrences. Event occurrence and publication must both be within 24 hours. Confirmation/conflict updates retain distinct cache identities; conflicting current role graphs or observations block impact. Research seeds remain disconnected from activation.
- Evidence expiry schedules a rerender; expired ownership clears actionable Waiver/Trade surfaces. A mandatory future D/ST consumes a free active slot first; only a full roster needs a legal second drop. Its cost is lost lineup projection, preserving occupied skill slots. QB opportunities use this same acquisition boundary.
