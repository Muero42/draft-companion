(function(root,factory){
  ...
  function weeklyRecordChronology(record,context={},now=Date.now()){
    ...
    if(record?.metric==='projected_points'&&(!Number.isFinite(record?.value)||record.value<0||record.value>WEEKLY_HALF_PPR_MAX[record.position]||record?.provenance?.providerScope!=='WEEKLY'||record?.provenance?.providerScoring!=='HALF'))return false;
  ...
  function projectionLane(payloads,{season,week,scoring:scoringInput,sleeperPlayers,verifiedAt=Date.now()}={}){
    ...
    for(const position of POSITIONS){
      ...
      else if(scoring(payload?.scoring)!==normalizedScoring)reason='WRONG_PROVIDER_SCORING';
