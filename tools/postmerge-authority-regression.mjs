...
const newV245Cases=[
  ...
  ['v248 rejects false rc4.200 production and device promotion',d=>{d['PITTI_CURRENT_STATE.json'].runtime.deployed_production_version='v11.8.0-rc4.200';d['PITTI_EXECUTION_LOCK.json'].runtime.latestAndroidVersionObserved='v11.8.0-rc4.200';d['PITTI_EXECUTION_LOCK.json'].runtime.testChallengerAndroidObserved=true;}],
...
const newV246Cases=[
  ['v248 generation regresses to v245',d=>d['PITTI_CURRENT_STATE.json'].handoff_generation='20260912T1317Z-v245'],
  ['v248 source regresses to rc4.198',d=>d['PITTI_CURRENT_STATE.json'].authority.source_candidate='v11.8.0-rc4.198'],
  ['v248 rc4.198 production commit replaced by later main',d=>d['PITTI_CURRENT_STATE.json'].runtime.production_deployment.source_commit='c9f7eb1a3dea788fb56eac517dab63b39ed9ef59'],
  ['v248 rc4.200 candidate implies device acceptance',d=>d['PITTI_CURRENT_STATE.json'].authority.rc4200_candidate.device_acceptance_proven=true],
  ['v248 preview mislabeled production',d=>d['PITTI_CURRENT_STATE.json'].runtime.preview_candidate='v11.8.0-rc4.200 PRODUCTION'],
  ['v248 PR state frozen',d=>d['PITTI_CURRENT_STATE.json'].authority.rc4200_candidate.pr_state='OPEN'],
  ['v248 lock snake-case timestamp stale',d=>d['PITTI_EXECUTION_LOCK.json'].updated_at='2026-09-12T13:17:00Z'],
];
...
console.log(`POSTMERGE_AUTHORITY_REGRESSION_PASS generation=v248 legacy=${legacyCases.length} new_v243=${newV243Cases.length} new_authority=${newAuthorityCases.length} external=${externalCases.length + 3} new_v244=${newV244Cases.length} new_v245=${newV245Cases.length} new_v248=${newV246Cases.length} + unchanged pre/post promotion fixtures + historical-scope preservation`);
