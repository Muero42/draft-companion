...
export const GENERATION='20260913T1800Z-v248';
...
export function validateAuthority(data) {
  ...
  check(generation===GENERATION,'CURRENT.handoff_generation','v248 generation required');
  ...
  check(l.runtime?.appVersion===SOURCE&&s.branch_locks?.source_baseline===SOURCE,'LOCK/SEAL.runtime','v248 source lock/checkpoint stale');
  check(s.branch_locks?.branch==='DYNAMIC_VERIFICATION_REQUIRED'&&s.branch_locks.reconciled_base_main==='DYNAMIC_EXTERNAL_EVIDENCE'&&String(s.branch_locks.codex_work||'').includes('v248 rc4.200')&&String(s.note||'').includes(DEVICE_VERDICT),'SEAL.rc4200 reconciliation','seal must separate rc4.200 source/package from rc4.198 production/device and rc4.195 rollback');
  ...
  check(l.updatedAt===c.updated_at&&l.updated_at===c.updated_at,'LOCK.timestamps','v248 timestamp alias drift');
  ...
  for(const p of docs) {
    ...
    for(const [i,line] of text.split(/\r?\n/).entries()) {
      ...
      if(/^## v248 CURRENT/.test(line)) historical=false;
