...
export function validateAuthority(d){
 ...
 const boundary=k.currentBoundary, commandDeployment=boundary?.productionDeployment;
 ...
 check(commandDeployment?.version==='v11.8.0-rc4.203'&&commandDeployment?.sourceCommit===MAIN&&commandDeployment?.deploymentId===DEPLOY&&commandDeployment?.status==='VERIFIED_SUCCESS'&&commandDeployment?.branch==='main','COMMAND: stale Production deployment object');
 check(k.seasonCompanion?.nextGate===AUTHORITY_GATE,'COMMAND: stale Season Companion gate');
