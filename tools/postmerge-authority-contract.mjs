...
export function validateAuthority(data) {
  ...
  check(c.authority?.repo==='Muero42/draft-companion'&&c.authority.branch==='main'&&c.authority.source_candidate==='v11.8.0-rc4.194','CURRENT.authority','canonical source identity drift');
  ...
  check(l.runtime?.appVersion==='v11.8.0-rc4.194'&&s.branch_locks?.source_baseline==='v11.8.0-rc4.194','LOCK/SEAL.runtime','runtime changed');
  ...
  check(c.runtime.android_acceptance_pending===true&&l.runtime.androidVerified===false,'rc4.194 device','local/source PASS cannot assert physical acceptance');
  ...
  for(const p of docs) {
    ...
    for(const token of ['rc4.194','rc4.193','rc4.169','UNKNOWN_REQUIRES_REVERIFICATION','MERGED/HISTORICAL']) check(text.includes(token),p,`current authority token missing: ${token}`);
