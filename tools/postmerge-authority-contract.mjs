...
export function validateAuthority(data) {
  ...
  check(/^20260911T\d{4}Z-v243$/.test(generation),'CURRENT.handoff_generation','v243 generation required');
  ...
  check(c.runtime.source_candidate_status==='PRODUCTION_DEPLOYED_PHYSICAL_PARTIAL_PASS_NOT_ACCEPTED'&&String(c.runtime.season_candidate||'').includes('source-merged and production-deployed')&&String(c.runtime.season_candidate||'').includes('NOT FULLY ACCEPTED'),'CURRENT.runtime.source merge','source, deployment and partial physical acceptance must remain distinct');
  check(String(c.runtime.local_candidate_package?.source_scope||'').includes('identity preserved independently of production/device state')&&!Object.hasOwn(c.runtime.local_candidate_package||{},'source_branch'),'CURRENT.runtime.local_candidate_package','package identity must not retain an operative pre-merge branch');
  ...
  check(s.branch_locks?.branch==='main'&&s.branch_locks.reconciled_base_main==='082d77003f6616e290146698641aebe63f37b8c2'&&String(s.branch_locks.codex_work||'').includes('PR #149')&&String(s.note||'').includes('PHYSICAL_PARTIAL_PASS_NOT_ACCEPTED'),'SEAL.rc4196 reconciliation','seal must record deployed rc4.196 partial canary and rc4.195 rollback');
  ...
  check(c.runtime.android_acceptance_pending===true&&l.runtime.androidVerified===false&&l.runtime.latestAndroidVersionObserved==='v11.8.0-rc4.196'&&String(l.runtime.androidAuthority).includes('NOT FULLY ACCEPTED')&&l.runtime.android_authority===l.runtime.androidAuthority,'device','rc4.196 partial observation must not become full acceptance');
  const packageSha='sha256:a654422c907e3127335c20df1956fc974442c3eb3be011a5d8eb1e9b71f4500d';
  for(const [label,pkg] of [['CURRENT',c.runtime.local_candidate_package],['LOCK',l.runtime.localCandidatePackage],['COMMAND',k.currentBoundary.localCandidatePackage]]) check(pkg?.version==='v11.8.0-rc4.196'&&pkg.files===17&&pkg.sha256===packageSha&&pkg.status==='PACKAGE_IDENTITY_VERIFIED_SEPARATE_FROM_DEPLOYMENT',`${label}.package`,'rc4.196 17-file package identity required');
  ...
  check(c.runtime.package_reference_scope.includes('rc4.196 byte-exact')&&k.currentBoundary.packageReferenceScope.includes('rc4.196 17-file package identity')&&l.runtime.preinstallHashSemantics.includes('rc4.196 local byte-exact'),'package scope','package scope must be current and non-deployment');
  const ci=c.runtime.ci;
  check(['candidate_package','release_contract_v2','project_guardrails'].every(key=>ci[key]==='HISTORICAL_PASS_EXACT_RC4196_SOURCE_MERGE_HEAD')&&ci.validated_code_head==='555487237c9075d5e5ceeb1fee196f4763f87cc3'&&ci.status==='HISTORICAL_SOURCE_MERGE_CI_NOT_CURRENT_MAIN_AUTHORITY'&&String(ci.evidence_scope).includes('not exact-main-head evidence for reconciled canonical main 082d770')&&ci.current_exact_main_head_ci==='DYNAMIC_EXTERNAL_VERIFICATION_REQUIRED_BEFORE_DEPENDENT_CONTINUATION_OR_PROMOTION','remote CI','555487 checks must remain historical-only; current 082d770 exact-head CI requires fresh dynamic verification');
  ...
  for(const p of docs) {
    ...
    for(const token of ['rc4.196','rc4.195','RC4.196_PHYSICAL_PARTIAL_PASS_NOT_ACCEPTED','a654422c907e3127335c20df1956fc974442c3eb3be011a5d8eb1e9b71f4500d']) check(text.includes(token),p,`current authority token missing: ${token}`);
    ...
    for(const [i,line] of text.split(/\r?\n/).entries()) {
      ...
      if(/^## v243 CURRENT/.test(line)) historical=false;
  ...
  const projectCurrent=(data['PITTI_PROJECT_STATE.md'].split('## v243 CURRENT')[1]||'');
  check(projectCurrent.includes('082d77003f6616e290146698641aebe63f37b8c2')&&projectCurrent.includes('PHYSICAL_PARTIAL_PASS_NOT_ACCEPTED')&&projectCurrent.includes('not arbitrary byte parity'),'PITTI_PROJECT_STATE.md v243 CURRENT','latest project-state authority must record exact deployment and partial device boundary');
  return errors;
}
