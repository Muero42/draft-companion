...
export function validateAuthority(data) {
  ...
  check(/^202609\d{2}T\d{4}Z-v241$/.test(generation),'CURRENT.handoff_generation','v241 generation required');
  ...
  check(c.runtime.source_candidate_status==='SOURCE_CANDIDATE_PR_ONLY'&&String(c.runtime.season_candidate||'').includes('rc4.196')&&String(c.runtime.season_candidate||'').includes('rc4.195 production/device accepted'),'CURRENT.runtime.source merge','merged source must remain separate from deployment and physical acceptance');
  check(c.runtime.local_candidate_package?.source_scope==='DRAFT_PR143_ONLY'&&!Object.hasOwn(c.runtime.local_candidate_package||{},'source_branch'),'CURRENT.runtime.local_candidate_package','package identity must not retain an operative pre-merge branch');
  ...
  check(s.branch_locks?.branch==='main'&&s.branch_locks.reconciled_base_main==='8aedc4ea3b7b71591cbc45f971ccf85fe5167074'&&String(s.branch_locks.codex_work||'').includes('Draft PR #143')&&String(s.note||'').includes('rc4.195 production/device acceptance'),'SEAL.rc4195 reconciliation','seal must record accepted rc4.195 and source-only PR #143');
  ...
  check(c.runtime.deployed_pages_head==='8aedc4ea3b7b71591cbc45f971ccf85fe5167074'&&c.runtime.deployed_pages_app_byte_parity_with_main===true&&l.runtime.deployedPagesVersion==='v11.8.0-rc4.195'&&l.runtime.deployedPagesAppByteParityWithMain===true&&l.runtime.mainGhPagesParity===true&&k.currentBoundary.deployedPagesVersion==='v11.8.0-rc4.195'&&k.currentBoundary.deployedPagesAppByteParityWithMain===true&&s.branch_locks.deployment_parity==='VERIFIED_RC4195','deployment','offline audit cannot assert deployment parity');
  check(c.runtime.android_acceptance_pending===true&&l.runtime.androidVerified===false,'rc4.195 device','local/source PASS cannot assert physical acceptance');
  ...
  for(const p of docs) {
    ...
    for(const token of ['rc4.195','rc4.193','rc4.169','UNKNOWN_REQUIRES_REVERIFICATION','MERGED/HISTORICAL']) check(text.includes(token),p,`current authority token missing: ${token}`);
