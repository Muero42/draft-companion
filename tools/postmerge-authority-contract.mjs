...
export const GENERATION='20260914T1727Z-v252';
const CHECKPOINT_AT='2026-09-14T17:27:00Z';
const SOURCE='v11.8.0-rc4.203';
const PROD='v11.8.0-rc4.202';
const PROD_COMMIT='4e2b9af1c8562c0273f39503c2c1c90a15acce00';
const DEPLOYMENT='d1e38b4b-27ed-4f87-bfa7-0e99f282de5d';
const DEVICE_VERSION='v11.8.0-rc4.202';
const DEVICE_VERDICT='RC4.202_PHYSICAL_FAIL_YEARLESS_PROJECTION_CHRONOLOGY_REJECTED';
const DEVICE_EVIDENCE='docs/PITTI_BRIDGE_HANDOFF_RC4202_PHYSICAL_PROJECTION_LANE_FAIL_2026-09-14.md';
...
export function validateAuthority(data) {
  ...
  check(generation===GENERATION,'CURRENT.handoff_generation','v252 generation required');
  check(c.updated_at===CHECKPOINT_AT&&l.updatedAt===CHECKPOINT_AT&&l.updated_at===CHECKPOINT_AT&&k.updated_at===CHECKPOINT_AT&&s.updated_at===CHECKPOINT_AT,'checkpoint timestamp','all v252 timestamp aliases must use the actual non-future repair checkpoint');
  ...
  for(const token of [SOURCE,PROD,PROD_COMMIT,DEPLOYMENT,DEVICE_VERDICT,'rc4.201 is the immediate prior Production/history','rc4.200','rc4.199','rc4.198','rc4.195']) check(String(c.source_authority||'').includes(token),'CURRENT.source_authority',`source authority chronology missing: ${token}`);
  ...
  const rc4202Alias=c.authority.rc4202_candidate;
  check(rc4202Alias?.checkpoint==='v252'&&rc4202Alias.version===PROD&&rc4202Alias.status==='MERGED/HISTORICAL_PRODUCTION_DEVICE_OBSERVED_FAILED'&&rc4202Alias.deployment_proven===true&&rc4202Alias.device_observation_proven===true&&rc4202Alias.device_acceptance_proven===false&&rc4202Alias.source_commit===PROD_COMMIT&&rc4202Alias.deployment_id===DEPLOYMENT,'CURRENT.authority.rc4202_candidate','rc4.202 alias must agree with its verified Production deployment and failed physical observation');
  const candidate=c.authority.rc4203_candidate;
  check(candidate?.checkpoint==='v252'&&candidate.version===SOURCE&&candidate.status==='SOURCE_CANDIDATE_NOT_PRODUCTION'&&candidate.deployment_proven===false&&candidate.device_observation_proven===false&&candidate.device_acceptance_proven===false&&candidate.pr_state==='DYNAMIC_VERIFICATION_REQUIRED'&&candidate.canonical_containment==='DYNAMIC_VERIFICATION_REQUIRED','CURRENT.authority.rc4203_candidate','rc4.203 must remain a dynamic, non-production source candidate');
  ...
  check(l.runtime?.appVersion===SOURCE&&s.branch_locks?.source_baseline===SOURCE,'LOCK/SEAL.runtime','v252 source lock/checkpoint stale');
  for(const token of [PROD,PROD_COMMIT,DEPLOYMENT,'device observed FAILED','NOT ACCEPTED']) check(String(l.runtime?.latestDeployedCandidate||'').includes(token),'LOCK.runtime.latestDeployedCandidate',`latest deployed alias missing rc4.202 authority: ${token}`);
  check(s.branch_locks?.branch==='DYNAMIC_VERIFICATION_REQUIRED'&&s.branch_locks.reconciled_base_main==='DYNAMIC_EXTERNAL_EVIDENCE'&&String(s.branch_locks.codex_work||'').includes('v252 rc4.203')&&String(s.note||'').includes(DEVICE_VERDICT),'SEAL.rc4201 reconciliation','seal must separate rc4.202 source/package from rc4.201 production/physical-failure evidence and rc4.195 rollback');
  ...
  for(const token of ['rc4.202',PROD_COMMIT,DEPLOYMENT,DEVICE_VERDICT]) check(String(s.branch_locks?.deployment_parity||'').includes(token),'SEAL.branch_locks.deployment_parity',`deployment alias missing rc4.202 identity: ${token}`);
  ...
  check(c.authority.rc4202_physical_failure?.evidence===DEVICE_EVIDENCE,'CURRENT.authority.rc4202_physical_failure.evidence','rc4.202 requires its dedicated immutable evidence record');
  ...
  check(followup?.lane==='CURRENT_RC4.203_CANDIDATE'&&String(followup.verification).includes('MUTABLE_LIVE_STATE')&&followup.head==='DYNAMIC_EXTERNAL_EVIDENCE','LOCK.liveVerificationTargets.followup','new rc4.203 follow-up PR must be a mutable exact-head verification target');
  check(pr163?.lane==='HISTORICAL_DISCOVERABLE_V248_ANCHOR_ONLY'&&String(pr163.verification).includes('MUST_NOT_OVERRIDE_V252')&&pr163.head==='DYNAMIC_EXTERNAL_EVIDENCE','LOCK.liveVerificationTargets.PR163','PR #163 must remain historical/discoverable only');
  ...
  check(String(sealedTargets?.draft_companion_rc4203_followup_pr||'').includes('current rc4.203 candidate')&&String(sealedTargets?.draft_companion_rc4203_followup_pr||'').includes('fresh live verification'),'SEAL.liveTargets.followup','sealed takeover must name dynamic rc4.203 follow-up lane');
  check(String(sealedTargets?.draft_companion_pr_163||'').includes('historical/discoverable v248')&&String(sealedTargets?.draft_companion_pr_163||'').includes('cannot override v252'),'SEAL.liveTargets.PR163','sealed takeover must constrain PR #163 to historical discovery');
  ...
  for(const token of [PROD_COMMIT,DEPLOYMENT,DEVICE_VERDICT,'PROJECTION_LANE_UNAVAILABLE','nicht geladen','QB UNAVAILABLE','RB UNAVAILABLE','WR UNAVAILABLE','TE UNAVAILABLE']) check(rc4200Evidence.includes(token),DEVICE_EVIDENCE,`immutable rc4.202 observation missing: ${token}`);
  ...
  check(c.authority.rc4202_production_deployment?.source_commit===PROD_COMMIT&&c.authority.rc4202_production_deployment.deployment_id===DEPLOYMENT&&c.authority.rc4202_production_deployment.branch==='main'&&c.authority.rc4202_production_deployment.device_acceptance_proven===false,'CURRENT.authority.rc4202_production_deployment','deployment success and failed device acceptance must remain separate');
  ...
  check(c.runtime.package_reference_scope.includes('rc4.203 source-byte-exact 17-file')&&k.currentBoundary.packageReferenceScope.includes('no cross-environment archive SHA identity')&&l.runtime.preinstallHashSemantics.includes('archive SHA is run/environment-scoped'),'package scope','package scope must be current, source-byte exact, and non-deployment');
  ...
  check(c.handoff.refresh_scope==='RC4.203_YEARLESS_PROJECTION_CHRONOLOGY_REPAIR_AFTER_RC4.202_PHYSICAL_FAIL','CURRENT.handoff.refresh_scope','rc4.203 repair scope must be anchored to the rc4.202 physical failure');
  check(l.updatedAt===c.updated_at&&l.updated_at===c.updated_at,'LOCK.timestamps','v252 timestamp alias drift');
  ...
  for(const p of docs) {
    ...
    for(const token of ['rc4.203','rc4.202','rc4.201','rc4.200','rc4.199','rc4.198','rc4.195',DEVICE_VERDICT,RC4199_VERDICT,'17-file','cross-environment']) check(text.includes(token),p,`current authority token missing: ${token}`);
    ...
    for(const [i,line] of text.split(/\r?\n/).entries()) {
      ...
      if(/^## v252 CURRENT/.test(line)) historical=false;
