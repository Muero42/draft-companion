...
export const GENERATION='20260913T1930Z-v249';
const SOURCE='v11.8.0-rc4.200';
const PROD='v11.8.0-rc4.199';
const PROD_COMMIT='2a62e52cb88187470542840053c72cd310b18e2b';
const DEPLOYMENT='48ab58ba-53d5-4b74-b7e6-16629256a9ee';
const DEVICE_VERSION='v11.8.0-rc4.199';
const DEVICE_VERDICT='RC4.199_PHYSICAL_FAIL_WEEKLY_PROJECTION_SEMANTIC_MISMATCH';
...
export function validateAuthority(data) {
  ...
  check(generation===GENERATION,'CURRENT.handoff_generation','v249 generation required');
  ...
  check(l.runtime?.appVersion===SOURCE&&s.branch_locks?.source_baseline===SOURCE,'LOCK/SEAL.runtime','v249 source lock/checkpoint stale');
  check(s.branch_locks?.branch==='DYNAMIC_VERIFICATION_REQUIRED'&&s.branch_locks.reconciled_base_main==='DYNAMIC_EXTERNAL_EVIDENCE'&&String(s.branch_locks.codex_work||'').includes('v249 rc4.200')&&String(s.note||'').includes(DEVICE_VERDICT),'SEAL.rc4200 reconciliation','seal must separate rc4.200 source/package from rc4.199 production/physical-failure evidence and rc4.195 rollback');
  ...
  check(c.runtime.package_reference_scope.includes('rc4.200 source-byte-exact 17-file')&&k.currentBoundary.packageReferenceScope.includes('no cross-environment archive SHA identity')&&l.runtime.preinstallHashSemantics.includes('archive SHA is run/environment-scoped'),'package scope','package scope must be current, source-byte exact, and non-deployment');
  const ci=c.runtime.ci;
  check(ci.validated_code_head==='DYNAMIC_EXTERNAL_EVIDENCE'&&ci.status==='LOCAL_VALIDATION_RECORDED_EXTERNAL_EXACT_HEAD_DYNAMIC'&&ci.candidate_package==='LOCAL_PASS_SOURCE_BYTE_PARITY'&&ci.current_exact_main_head_ci==='DYNAMIC_EXTERNAL_VERIFICATION_REQUIRED_BEFORE_DEPENDENT_CONTINUATION_OR_PROMOTION','remote CI','local rc4.200 verification and dynamic external exact-head boundary required');
  ...
  check(l.updatedAt===c.updated_at&&l.updated_at===c.updated_at,'LOCK.timestamps','v249 timestamp alias drift');
  ...
  for(const p of docs) {
    ...
    for(const token of ['rc4.200','rc4.199','rc4.198','rc4.195',DEVICE_VERDICT,'17-file','cross-environment']) check(text.includes(token),p,`current authority token missing: ${token}`);
    ...
    for(const [i,line] of text.split(/\r?\n/).entries()) {
      ...
      if(/^## v249 CURRENT/.test(line)) historical=false;
