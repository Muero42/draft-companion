...
const activeVersion=(app.match(/const APP_VERSION='v11\.8\.0-rc4\.(\d+)'/)||[])[1];assert.ok(Number(activeVersion)>=84,'active runtime below rc4.84 feature gate');
...
console.log('RC484_DRAFT_CRITICAL_PASS');

// rc4.84 bounded late-WR saturation challenger invariants.
...
assert.ok(evidenceAnalyzer.includes("assert.equal(e.appVersion"),'Evidence analyzer appVersion guard missing');
