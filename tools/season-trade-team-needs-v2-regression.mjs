...
{
  const x=trade.evaluateOffer({mine,opponent,give:[mine[1]],get:[opponent[1]],evidence:fresh,slots,maxActive:15,preWeek1:true});
  ...
  const y=trade.evaluateOffer({mine,opponent,give:[mine[1]],get:[opponent[1]],evidence:explained,slots,maxActive:15,preWeek1:true});
...
// An offered position the opponent cannot use and replacement loss on either side reject.
{
  ...
  const noNeed=trade.evaluateOffer({mine:ours,opponent:stacked,give:[ours[0]],get:[stacked[0]],evidence:ev,slots,maxActive:15,preWeek1:false});
  assert.equal(noNeed.actionable,false);assert.notEqual(noNeed.reason,'BILATERAL_UTILITY_PASS');
  const replacement=trade.evaluateOffer({mine:ours,opponent:stacked,give:[ours[2]],get:[stacked[2]],evidence:ev,slots,maxActive:15,preWeek1:false});
...
// Reserve does not create capacity; a 1-for-2 into a full active roster is illegal.
{
  ...
  const x=trade.evaluateOffer({mine:full,opponent:opp,give:[full[0]],get:[opp[0],opp[1]],evidence:ev,slots,maxActive:15,preWeek1:false});
...
// Missing/stale current values can never produce an actionable offer.
{
  ...
  const x=trade.evaluateOffer({mine,opponent,give:[mine[1]],get:[opponent[1]],evidence:stale,slots});
  assert.equal(x.status,'MONITOR');assert.equal(x.actionable,false);assert.equal(x.reason,'STALE_VALUATION');
}

const app=fs.readFileSync(new URL('../app.js',import.meta.url),'utf8');
assert.ok(app.includes('draftPickByPlayer=new Map((picks||[]).map(pk=>[String(pk.player_id),Number(pk.pick_no)]))'));
assert.ok(app.includes('Sleeper roster_id is a league roster identifier, NOT the historical draft slot.'));
assert.ok(!app.includes('function tradeOfferCandidates('),'simplistic target-first 1-for-1 generator must be removed');
