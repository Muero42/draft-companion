...
const packageTool=read('tools/package-reextract.mjs'),packageWorkflow=read('.github/workflows/release-contract-v2-package.yml');
...
must(RUNTIME_FILES.includes('lineup-start-sit-v2.js'),'runtime manifest missing Start/Sit optimizer');
must(RUNTIME_FILES.includes('game-context-v1.js'),'runtime manifest missing game-context adapter');
must(packageWorkflow.includes("import {RUNTIME_FILES} from './tools/runtime-files.mjs'"),'candidate package workflow must consume canonical runtime manifest');
must(packageWorkflow.includes('test "${#RUNTIME_FILES[@]}" -eq 17'),'candidate package workflow must guard canonical 17-file count');
must(!packageWorkflow.includes('test "$(unzip -Z1 "dist/$PKG" | wc -l)" -eq 15'),'stale 15-file candidate package assertion resurrected');
