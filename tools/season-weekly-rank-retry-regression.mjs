...
async function run({projectionFailures={},rankFailures={},existingRetry=0}={}){
  const cache=new Map(existingRetry?[[retryKey,existingRetry]]:[]);let calls=0,writes=0;
  ...
  const api={CACHE_KEY:'weekly',buildSnapshot:({rankingPayloads})=>Object.keys(rankingPayloads||{}).length?final:projection,atomicWrite(storage,value){writes++;cache.set(this.CACHE_KEY,value);return value}};
  const context={Date:{now:()=>1000},Number,String,Array,Object,Math,Promise,console,WEEKLY_PROJECTION_POSITIONS:positions,SEASON_RANKING_AUTO_MS:1,SEASON_RANKING_RETRY_MS:1,seasonRankingRefreshBusy:false,store,els:{season:{value:'2026'},scoring:{value:'HALF'},seasonRefreshEvidenceBtn:{},seasonRankingStatus:{}},navigator:{onLine:true},lastDraftContext:{players:{},season:{}},localStorage:{},currentSleeperNflWeek:async()=>1,
    fpProxyRequest:async path=>{calls++;const position=new URLSearchParams(path.split('?')[1]).get('position'),failure=path.includes('consensus-rankings')?rankFailures[position]:projectionFailures[position];if(failure?.reject)throw Object.assign(new Error('malformed 429'),failure);if(failure)return{ok:false,status:failure.status,retryAfterMs:failure.retryAfterMs,data:null};return{ok:true,status:200,data:{}}},
  ...
  return{result:await context.refreshSeasonRankings({force:true}),cache,calls,writes};
}
