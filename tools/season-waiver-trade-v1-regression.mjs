...
const sandbox={Date,console,globalThis:{},lastDraftContext:{season,players:playerRows},SLEEPER_NON_STARTER_SLOTS:new Set(['BN','IR','TAXI']),BOONE_TRADE_VALUE_CACHE_KEY:'pitti.boone-trade-values.v1.current',validateBooneTradeValueSnapshot:()=>({ok:false}),store:{get:(k,f)=>cache.get(k)??f},rankFor:name=>({rank:rank.get(name)??100}),weeklyLineupEvidence:p=>({consensus:rank.get(p.name)??null}),managerProfile:()=>({historical:{positions:{WR:{finalCount:2}}}}),clamp:(v,min,max)=>Math.max(min,Math.min(max,v))};
...
assert.equal(summary.bidBasis,'ORIGINAL_FAAB_BUDGET','all percentages use original league budget');
assert.equal(summary.originalBudget,100);assert.equal(summary.ownRemaining,5);
assert.equal(summary.bidHigh,5,'absolute bid uses original-budget percentage then caps at actual remaining FAAB');
