...
const files=['index.html','app.js','decision-policy.js','weekly-evidence-v2.js','lineup-start-sit-v2.js','game-context-v1.js','boone-trade-values-v1.mjs','styles.css','manifest.webmanifest','sw.js','_worker.js','icon.svg','live-surface-v3.js','live-surface-v3.css','expert-board-export.js','expert-v2-board.js','expert-v3-board.js'];
...
assert.equal(fs.readdirSync(extract).length,17);
for(const f of files)assert(fs.readFileSync(path.join(extract,f)).equals(fs.readFileSync(path.join(root,f))),f+' byte parity');
for(const f of files.filter(f=>/\.m?js$/.test(f)))execFileSync(process.execPath,['--check',path.join(extract,f)]);
...
const receipt={version,status:'PACKAGED_ONLY_NOT_DEPLOYED',files:17,sha256:crypto.createHash('sha256').update(fs.readFileSync(archive)).digest('hex'),archive};
