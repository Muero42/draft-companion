...
const candidatePreflight=process.env.PITTI_CANDIDATE_PREFLIGHT==='1'?'1':'0';
const tree=process.env.PITTI_VALIDATED_TREE||git('write-tree'),run=(file,...args)=>execFileSync(process.execPath,[file,...args],{stdio:'inherit',env:{...process.env,PITTI_SKIP_SEAL_INTEGRITY:'0',PITTI_CANDIDATE_PREFLIGHT:candidatePreflight}});
...
for(const t of ['release-contract-v2','runtime-startup-contract','live-presentation-behavior','season-decision-engine-regression','season-waiver-trade-v1-regression','boone-trade-value-runtime-regression','season-adversarial-review-regression','pitti_guardrail_check'])execFileSync(process.execPath,['tools/'+t+'.mjs'],{cwd:harness,stdio:'inherit',env:{...process.env,PITTI_SKIP_SEAL_INTEGRITY:'0',PITTI_CANDIDATE_PREFLIGHT:candidatePreflight}});
