...
try{
  ...
  let attempt=0;globalThis.fetch=async()=>{attempt++;const context={season:{year:2026,type:2},week:{number:2}};if(attempt===1)return new Response(JSON.stringify({...context,events:[]}),{status:200,headers:{'content-type':'application/json'}});return new Response(JSON.stringify({...context,events:Array.from({length:13},(_,i)=>({id:String(i+1)}))}),{status:200,headers:{'content-type':'application/json'}})};
