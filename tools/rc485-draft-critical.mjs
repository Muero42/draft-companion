...
const activeVersion=(app.match(/const APP_VERSION='v11\.8\.0-rc4\.(\d+)'/)||[])[1];assert.ok(Number(activeVersion)>=85,'active runtime below rc4.85 feature gate');
...
assert.match(idx,/v11\.8\.0-rc4\.(?:85|8[6-9]|9\d|[1-9]\d{2,})/);
assert.match(sw,/v11\.8\.0-rc4\.(?:85|8[6-9]|9\d|[1-9]\d{2,})/);
assert.match(manifest,/v11\.8\.0-rc4\.(?:85|8[6-9]|9\d|[1-9]\d{2,})/);
...
assert.equal(safetyPromotionEligiblePolicy({pos:'WR',counts:{WR:6},rank:105,adp:110,current:100}),false);
assert.equal(safetyPromotionEligiblePolicy({pos:'WR',counts:{WR:6},rank:105,adp:85,current:100}),true);
...
console.log('RC485_DRAFT_CRITICAL_PASS');
...
assert.ok(evidenceAnalyzer.includes("assert.equal(e.appVersion"),'Evidence analyzer version guard missing');
