...
- Current handoff generation = **20260907T1900Z-v236**. This checkpoint records the repair after the performed v233 read-only audit failed closed on residual state pointers.
- Current mode = **POST_DRAFT_SEASON_COMPANION**.
- Canonical source candidate = **rc4.191 preserving merged PR #118/#132; not deployed or physically accepted**. Verify the exact current main HEAD dynamically; a tracked checkpoint must not hard-code its own containing commit as immutable current authority.
...
- Source candidate v11.8.0-rc4.191 preserves merged PR #118/#132; latest physical Android rc4.188 PASS; rollback rc4.169; deployment parity UNKNOWN_REQUIRES_REVERIFICATION. PR #121 / v233 MERGED/HISTORICAL. No deployment or device acceptance is claimed.
