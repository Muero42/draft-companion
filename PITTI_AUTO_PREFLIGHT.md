...
- Current handoff generation = **20260910T1139Z-v238**. This checkpoint records the cache-identity repair required after the weekly-projection diagnostic reached production under the already-used rc4.192 identity.
- Current mode = **POST_DRAFT_SEASON_COMPANION**.
- Canonical source candidate = **rc4.193 cache-distinct weekly-projection diagnostic candidate; not deployed or physically accepted**. Verify the exact current main HEAD dynamically; a tracked checkpoint must not hard-code its own containing commit as immutable current authority.
...
- Deployment evidence = **rc4.192 production serves the merged weekly diagnostic, but with a reused PWA cache identity**. rc4.193 deployment parity remains pending. Never infer device acceptance from source merge or a fresh-browser check.
...
- Source candidate v11.8.0-rc4.193 is the cache-safe successor for the merged authenticated weekly-projection diagnostic; latest physical Android rc4.188 PASS; rollback rc4.169. rc4.193 deployment/device acceptance is not claimed.
