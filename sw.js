const CACHE='draft-companion-v11.8.0-rc4.52';
const BACKUP_CACHE='draft-companion-backup-export-v1';
const ASSETS=['./','./index.html','./styles.css','./app.js?v=11.8.0-rc4.52','./manifest.webmanifest','./icon.svg'];
const BASE='v11.8.0-rc4.46',TARGET='v11.8.0-rc4.52';
function patchApp(s){
  s=s.replaceAll(BASE,TARGET).replaceAll('11.8.0-rc4.46','11.8.0-rc4.52');
  ...
  if(!s.includes(old1)||!s.includes(old2)||!s.includes(old3))throw new Error('rc4.52 patch anchors missing');
...
function patchText(path,s){
  ...
  if(path.endsWith('/index.html')||path.endsWith('/manifest.webmanifest')||path.endsWith('/'))return s.replaceAll(BASE,TARGET).replaceAll('11.8.0-rc4.46','11.8.0-rc4.52');
  return s;
}
